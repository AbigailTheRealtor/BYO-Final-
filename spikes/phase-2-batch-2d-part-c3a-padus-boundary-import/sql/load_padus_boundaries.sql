-- ============================================================================
-- load_padus_boundaries.sql  —  AUTHORED, NOT RUN
-- Spatial Intelligence Platform · Phase 2 Batch 2D Part C3a
-- PAD-US protected-area load: boundaries_staging -> boundaries + boundaries_parts
-- ============================================================================
--
...
--
-- Flow: append staged rows to `boundaries`, then derive `boundaries_parts` via
-- ST_Subdivide. No existing row is mutated.
--
-- Run (later, Class-2): psql "$SPATIAL_DATABASE_URL" \
--   -v corpus_version=padus-<release> -f load_padus_boundaries.sql
-- ----------------------------------------------------------------------------

-- 1. Append protected-area polygons to boundaries (id bigserial is assigned here).
