-- ============================================================================
-- load_tiger_boundaries.sql  —  AUTHORED, NOT RUN
-- Spatial Intelligence Platform · Phase 2 Batch 2D Part C3b
-- Census TIGER load: boundaries_staging -> boundaries + boundaries_parts
-- ============================================================================
--
...
--
-- Flow: append the four TIGER kinds to `boundaries`, then derive `boundaries_parts`
-- via ST_Subdivide. No existing row is mutated. Counties and ZCTAs have far more
-- vertices than PAD-US units, so the <=256-vertex subdivision matters more here.
--
-- Run (later, Class-2): psql "$SPATIAL_DATABASE_URL" \
--   -v corpus_version=tiger-<vintage> -f load_tiger_boundaries.sql
-- ----------------------------------------------------------------------------

-- 1. Append TIGER boundaries (id bigserial is assigned here).
...
WHERE s.kind IN ('county', 'place', 'zcta', 'school_district');
...
WHERE b.corpus_version = :'corpus_version'
  AND b.kind IN ('county', 'place', 'zcta', 'school_district');
