...
- meta `workflow_type='offer_listing'` (Offer Listing sets this, e.g. `SellerOfferListing.php:3184`)
- fallback heuristic `OFFER_LISTING_META_KEYS` (`SellerOfferListingController.php:23-33`) — historically included `auction_type` + `brokerage_relationship`, which Hire Agent ALSO writes → draft-mixing bug (Hire rows leaking into Offer "My Listings"/drafts). Discriminator queries live in `DashboardController.php` (~`:73-76` counts, `:549-560` My Listings).
