...
{{-- 9B-2 (restore): the discrete "Acceptable Counties" / "Acceptable State" controls are
     kept as a fallback surface alongside the Search Areas map below. The committed 9B-2
     logic is unchanged — legacy prefill seeds the blob from these discrete values on load,
     and the blob→discrete write-back (hydrateDiscreteLocationFromBlob) mirrors them into
     the `counties` / `state` meta on save. Removing this UI entirely is deferred to 9B-3. --}}
<!-- Acceptable Counties -->
