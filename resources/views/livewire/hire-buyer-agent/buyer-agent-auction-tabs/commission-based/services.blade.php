...
<div style="background:yellow;padding:5px;margin-bottom:10px;font-weight:bold;">
    DEBUG SERVICES: property_type = "{{ $property_type ?? 'NULL' }}" | type = "{{ $type }}" | isIncome = {{ $isIncome ? 'YES' : 'NO' }} | isResidential = {{ $isResidential ? 'YES' : 'NO' }}
</div>
...
{{-- Income Property (2-4 Units) Services --}}
@if ($isIncome)
    @include('livewire.hire-buyer-agent.buyer-agent-auction-tabs.commission-based.services.income')

{{-- Residential Property Services --}}
@elseif ($isResidential)
    @include('livewire.hire-buyer-agent.buyer-agent-auction-tabs.commission-based.services.residential')
@endif
