...
<div class="buyerOfferContentDetails">
    <div class="container">
        <div class="mb-4">
            ...
            <p class="text-muted small">Properties where Sellers are accepting competitive offers. Compare bids and terms transparently — Residential, Income, Commercial, Business Opportunity, and Vacant Land.</p>
                ...
                <div class="card rounded-3" style="overflow: hidden;">
                    <div style="min-height: 202px;">
                        <img
                            ...
                            class="card-img-top" alt="{{ $pa->address }}" style="width: 100%; height:202px; object-fit: cover;">
                    </div>
                    <div class="card-body pb-2 pt-2">
                        <div style="min-height: 56px;">
                            <h5 class="card-title"><a href="{{ route('view-pl', $pa->id) }}">{{ $pa->address }}</a></h5>
                        </div>
                        <div class="houseDetails mb-1">
                            <span class="d-inline-flex align-items-center gap-2 flex-wrap">
                                ...
                                <span class="d-inline-flex align-items-center gap-1">
                                    ...
                                    <b>{{ @$pa->bathroom->name }}</b>
                                </span>
                                <span class="d-inline-flex align-items-center gap-1">
                                    <img src="{{ asset('assets/fontawesome/svgs/thin/ruler-triangle.svg') }}" alt="sqft icon" width="15">
                                    <b>{{ number_format($pa->heated_sqft, 0, '', ',') }}</b> Sq Ft
                                </span>
                            </span>
                            <div class="text-muted small mt-1">For Sale</div>
                        </div>

                        @php $ptype = @$pa->property_type[0]; @endphp
                        <p class="card-text mb-1">
                            <span class="badge bg-secondary">{{ @$ptype->property_type->name }}</span>
                            <span class="float-end text-muted small"><b>MLS</b> #{{ $pa->mls_id }}</span>
                        </p>
                        ...
                        <p class="m-0">
                            ...
                            <b class="badge bg-info timer-{{ $pa->id }}" data-time="20s">
                                {{ round($pa->auction_length) <= 0 ? 'No Time Limit' : $diff . 'd ' . $start->diff($end)->format('%H:%I:%S') }}
                            </b>
                        </p>
                    </div>
