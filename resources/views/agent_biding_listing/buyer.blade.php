...
.agent-bid-card { border-left: 3px solid #6f42c1 !important; }
</style>
@endpush
@section('content')
                    ...
                    <div class="rightCol col-sm-12 col-md-9 col-lg-9">
                        <div class="container mt-4 agent-bid-page">
                            ...
                            <div class="mb-3">
                                ...
                                <h4 class="mt-2 mb-0">Hire Buyer's Agent — My Bids</h4>
                                <p class="page-subtitle mb-0">Listings where you've placed bids to represent Buyers.</p>
                            </div>
                            <hr class="mt-2 mb-3">

                            {{-- Status Filter --}}
                            <div class="d-flex gap-2 mb-4 flex-wrap">
                                <a href="{{ route('buyer.biding.auctions.list') }}?type=2"
                                   class="btn btn-sm {{ $type == '2' ? 'text-white' : 'btn-outline-secondary' }}"
                                   style="{{ $type == '2' ? 'background:#049399;border-color:#049399;' : '' }}">
                                    Live <span class="badge bg-white text-dark ms-1">{{ $liveCount }}</span>
                                </a>
                                <a href="{{ route('buyer.biding.auctions.list') }}?type=1"
                                   class="btn btn-sm {{ $type == '1' ? 'text-white' : 'btn-outline-secondary' }}"
                                   style="{{ $type == '1' ? 'background:#6c757d;border-color:#6c757d;' : '' }}">
                                    Pending <span class="badge bg-white text-dark ms-1">{{ $pendingApprovalCount }}</span>
                                </a>
                                <a href="{{ route('buyer.biding.auctions.list') }}?type=3"
                                   class="btn btn-sm {{ $type == '3' ? 'text-white' : 'btn-outline-secondary' }}"
                                   style="{{ $type == '3' ? 'background:#28a745;border-color:#28a745;' : '' }}">
                            ...
                            @if($auctions->isEmpty())
                                <div class="text-center text-muted py-5">
                                    ...
                                    <p class="small mb-0">Switch the filter above or browse live Buyer's Agent listings to place a bid.</p>
                                </div>
                            @else
                                @php
                                    // Build 4 Phase 1: N+1 guard — memoize profile by property_type key.
                                    $_buyerProfCache = [];
                                    $_getBuyerProf   = function(string $ptKey) use (&$_buyerProfCache): array {
                                        if (!array_key_exists($ptKey, $_buyerProfCache)) {
                                            $prof = \App\Models\AgentDefaultProfile::findForAgentWithFallback(
                                                auth()->id(), 'buyer', $ptKey
                                            );
                                            $_buyerProfCache[$ptKey] = $prof ? ($prof->profile_data ?? []) : [];
                                        }
                                        return $_buyerProfCache[$ptKey];
                                    };
                                @endphp
                                ...
                                @php
                                    ...
                                    $listingId = $auction->listing_id ?? 'BAA-'.$auction->id;
                                    ...
                                    $_bAgentPtKey    = strtolower(str_replace(' ', '_', $propType)) ?: 'residential';
                                    $_bAgentProfData = $_getBuyerProf($_bAgentPtKey);

                                    $matchScore = \App\Helpers\BuyerBidMatchScoreHelper::calculate(
                                        $auctionBaselineData, $bidData, null, $propType, $_bAgentProfData
                                    );
                                    ...
                                    $compatScore  = \App\Services\CompatibilityScoreService::score($auctionBaselineData, $bidData, 'buyer', $propType);

                                    // Phase 1: build human-readable explanation (label always rendered; reasons conditional on profile data)
                                    $matchExplanation = \App\Services\AgentMatchExplanationBuilder::build($matchScore, $_bAgentProfData);
                                @endphp
                                ...
                                <div class="card mb-3 agent-bid-card" style="border-radius:8px;border:1px solid #dee2e6;">
                                    ...
                                    <div class="card-footer d-flex justify-content-between align-items-center flex-wrap gap-2"
                                        ...
                                        <div class="d-flex gap-2 flex-wrap justify-content-end">
                                            <a href="{{ route('buyer.agent.bid.detail', $userBid->id) }}"
                                            ...
                                            @elseif($bidStatus === 'Countered')
                                                <a href="{{ route('agent.buyer.agent.auction.bid.view-counter', $userBid->id) }}"
                                                   class="btn btn-sm" style="background:#fff3cd;color:#856404;border:1px solid #ffc107;font-size:.8rem;">
                                                    <i class="fa-solid fa-right-left me-1"></i>View Counter Terms
                                                </a>
                                            @elseif($bidStatus === 'Active')
                                                <a href="{{ route('agent.buyer.agent.auction.bid', $auction->id) }}"
                                                   class="btn btn-sm btn-outline-secondary" style="font-size:.8rem;">
