                        ...
                        <div class="container mt-4 agent-bid-page">
                            ...
                            @else
                                @php
                                    ...
                                    $_landlordProfCache = [];
                                    $_getLandlordProf   = function(string $ptKey) use (&$_landlordProfCache): array {
                                        if (!array_key_exists($ptKey, $_landlordProfCache)) {
                                            $prof = \App\Models\AgentDefaultProfile::findForAgentWithFallback(
                                                auth()->id(), 'landlord', $ptKey
                                            );
                                            $_landlordProfCache[$ptKey] = $prof ? ($prof->profile_data ?? []) : [];
                                        }
                                        return $_landlordProfCache[$ptKey];
                                    };
                                @endphp
                                ...
                                @php
                                    ...
                                    $_lAgentPtKey    = strtolower(str_replace(' ', '_', $propType)) ?: 'residential';
                                    $_lAgentProfData = $_getLandlordProf($_lAgentPtKey);

                                    $matchScore = \App\Helpers\LandlordBidMatchScoreHelper::calculate(
                                        $auctionBaselineData, $bidData, null, $propType, $_lAgentProfData
                                    );
                                    ...
                                    $readiness    = \App\Services\MatchReadinessService::evaluate($bidData, 'landlord');
                                    $compatScore  = \App\Services\CompatibilityScoreService::score($auctionBaselineData, $bidData, 'landlord', $propType);

                                    // Phase 1: build human-readable explanation (label always rendered; reasons conditional on profile data)
                                    $matchExplanation = \App\Services\AgentMatchExplanationBuilder::build($matchScore, $_lAgentProfData);
                                @endphp
