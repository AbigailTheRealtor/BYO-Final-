...
@section('content')
  ...
  <div class="buyerOfferContentDetails">
    <div class="container">
      <div class="mb-4">
        <h4 class="fw-bold">Hire a Seller's Agent</h4>
        <p class="text-muted small">Browse listings where Sellers are looking for agent representation. Agents bid to be hired — compare commission, services, and marketing strategy.</p>
        <p class="text-muted small mb-0"><strong>{{ $count }}</strong> listing{{ $count != 1 ? 's' : '' }} found</p>
      </div>

      {{-- Filters row (display only — no backend logic changed) --}}
            ...
            <div class="card rounded-3" style="overflow: hidden;">
              <div class="card-body pb-2 pt-2">
                <div style="min-height: 56px;">
                  <p class="mb-1 fw-semibold text-muted small text-uppercase" style="letter-spacing:.04em;font-size:.7rem;">Seller &bull; Residential / Income / Commercial / Business Opportunity / Vacant Land</p>
                  <h5 class="card-title mb-1"><a href="{{ route('seller.agent.auction.detail', @$auction->id) }}">{{ @$auction->address }}</a></h5>
                </div>
                ...
                <div class="houseDetails mb-2">
                  <span class="d-inline-flex align-items-center gap-2">
                    <span class="d-inline-flex justify-content-center align-items-center gap-1">
                      ...
                      <b>{{ @$auction->get->bedrooms ?: '—' }}</b> Beds
                    </span>
                    <span class="d-inline-flex justify-content-center align-items-center gap-1">
                      <img src="{{ asset('assets/fontawesome/svgs/thin/bath.svg') }}" alt="bath icon" width="15">
                      <b>{{ @$auction->get->bathrooms ?: '—' }}</b> Baths
                    </span>
                  </span><br>
                  <span class="text-muted small">Seller's Agent Required</span>
                </div>
          ...
          <div class="col-12">
            <div class="card border-0 bg-light rounded-3 p-5 text-center">
              <div class="text-muted">
                <i class="fa-solid fa-gavel fa-3x mb-3 opacity-25"></i>
                <h5 class="fw-semibold">No listings found</h5>
                <p class="small mb-0">No Sellers are currently looking for an agent in this category. Check back soon or broaden your search.</p>
              </div>
            </div>
          </div>
        @endforelse
