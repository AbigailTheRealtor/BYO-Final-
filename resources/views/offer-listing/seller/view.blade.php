...
#solAiModal .modal-footer .btn-primary:hover,
#solAiModal .modal-footer .btn-primary:focus {
    background-color: #1d4ed8 !important;
    border-color: #1d4ed8 !important;
    color: #ffffff !important;
}
...
<div class="container py-4 sol-view-page">
    ...
    <div class="card section-card">
        ...
        <div class="card-body">
            ...
            {{-- Seller Financing --}}
            @if($hasSellerFin)
            @if($str('seller_financing_type') || $str('seller_down_payment_amount') || $str('interest_rate') || $str('loan_duration') || $str('prepayment_penalty_amount') || $str('balloon_payment') || $str('balloon_payment_amount') || $str('balloon_payment_date') || $str('seller_amortization_type') || $str('seller_payment_frequency') || $str('seller_late_fee_amount'))
            <hr>
    ...
    @php
        ...
        $hasFinancial = in_array($finPropType, ['Income', 'Commercial', 'Business'])
            && ($str('minimum_annual_net_income') || $str('minimum_cap_rate') || $str('gross_annual_income')
                ...
                || $str('business_location_leased') || ($finPropType === 'Business' && $str('real_estate_purchase')));
    @endphp
    @if($hasFinancial)
    <div class="card section-card">
            ...
            <div class="row">
                <div class="col-md-6">
                    ...
                    @elseif($finPropType === 'Business')
                        ...
                        {{-- What the sale includes — the Business form's own question. It used to print
                             inside the Seller Financing block, so a guest saw it only when seller
                             financing happened to be offered. Ask AI answers it from this row. --}}
                        {!! $row('Business & Real Estate Purchase', $str('real_estate_purchase')) !!}
    ...
    @include('offer-listing.partials._ask-ai-question-modal', [
        'role'          => 'seller',
        'prefix'        => 'sol',
        'modalId'       => 'solAiModal',
...
(function () {
    ...
    /* Ask AI is selection-based: the modal's question picker (js/ask-ai/question-picker.js) replaced the free-text submit script. */
