...
@section('content')
    ...
    <div class="buyerOfferContentDetails">
        <div class="container">
            <p><span><b>Browse Seller Property Listings</b></span> <span><i>{{ $count }} results</i></span></p>
                            ...
                            <div class="card-body pb-2 pt-2">
                                <div style="min-height: 56px;">
                                    <h5 class="card-title w-75">
                                        <a href="{{ route('offer.listing.seller.view', @$auction->id) }}">{{ @$auction->title ?: @$auction->address }}</a>
                                    </h5>
                                </div>

                                <div class="houseDetails mb-1">
                                    <p class="mb-1 fw-bold text-muted small">Seller Listing &bull; {{ $propertyTypeLabel }}</p>
                                    ...
                                    <p class="mb-1">&#x1F3E0; Property Listing</p>
                            ...
                            <div class="card-footer bg-light">
                                <div class="row">
                                    ...
                                    <div class="col-6 right text-end">
                                        @php
                                            $_sb = preg_replace('/[^0-9.]/', '', (string)(@$auction->get->budget ?? ''));
                                        @endphp
                                        @if($_sb !== '' && is_numeric($_sb))
                                            <b>${{ number_format((float)$_sb, 0) }}</b>
                                        @elseif(!empty(@$auction->get->budget))
                                            <b>{{ @$auction->get->budget }}</b>
                                        @endif
                                    </div>
                                </div>

                                {{-- Renders nothing at all while the feature flag is off,
                                     for a guest-ineligible account, or for a viewer whose
                                     market this listing is not. --}}
