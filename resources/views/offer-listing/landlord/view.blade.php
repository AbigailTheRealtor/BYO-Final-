...
/* ---- Ask AI modal submit button — override Tailwind preflight ----
   ...
   ----------------------------------------------------------------- */
#lolAiModal .modal-footer .btn-primary {
    background-color: #2563eb !important;
    border-color: #2563eb !important;
    color: #ffffff !important;
}
#lolAiModal .modal-footer .btn-primary:hover,
#lolAiModal .modal-footer .btn-primary:focus {
            ...
            <div class="col-lg-4">
                <div class="lol-hero-summary">
                    ...
                    <div class="mt-2 d-flex align-items-center gap-2 flex-wrap">
                        ...
                        @if($bpClosed)
                            <span class="badge bg-secondary" style="font-size:.8rem;">Bidding Closed</span>
                        @else
    ...
    <div class="col-lg-3 d-none d-lg-block">
        <div class="lol-sticky-card">
            ...
            @php
                ...
                $_lolSidebarEndDate = $bpEndsAtDisplay?->format('M j, Y');
            @endphp
            ...
            <div class="modal-content" style="border-radius:.85rem;overflow:hidden;border:none;">
                ...
                <div class="modal-body p-4">
                    ...
                    <div id="lolAiSuggestions"
                        ...
                        @foreach($__lolCategorized as $__catKey => $__catItems)
                        <div class="mb-2">
                        ...
                        <div class="ask-ai-chip-wrap">
                            @foreach($__lolAiSuggestions as $__sq)
                            @php $__sqLabel = mb_strlen($__sq['question']) > 80 ? mb_substr($__sq['question'], 0, 79) . '…' : $__sq['question']; @endphp
                            <button type="button" role="button" class="ask-ai-chip"
                                    data-question="{{ $__sq['question'] }}"
                                    data-category="{{ $__sq['category'] ?? '' }}"
                    ...
                    <div id="lolAiConversation" style="display:none;max-height:260px;overflow-y:auto;margin-bottom:.75rem;padding:.6rem .75rem;background:#f8fafc;border:1px solid #e2e8f0;border-radius:.5rem;scroll-behavior:smooth;"></div>
                    {{-- V2: inline lead-capture form (shown when CTA action button clicked) --}}
                    <div id="lolAiLeadForm" style="display:none;margin-top:.5rem;margin-bottom:.75rem;background:#f0f9ff;border:1px solid #bae6fd;border-radius:.5rem;padding:.85rem 1rem;">
                        <div style="font-size:.78rem;font-weight:700;color:#0369a1;margin-bottom:.5rem;">
                            <i class="fa-solid fa-paper-plane me-1" aria-hidden="true"></i><span id="lolAiLeadTitle">Contact the Agent</span>
                        </div>
                        <input type="hidden" id="lolAiLeadActionKey">
                        <div class="mb-2">
                            <input type="text" class="form-control form-control-sm" id="lolAiLeadName" placeholder="Your name" maxlength="100" autocomplete="name">
                        </div>
                        <div class="mb-2">
                            <input type="email" class="form-control form-control-sm" id="lolAiLeadEmail" placeholder="Your email" maxlength="200" autocomplete="email">
                        </div>
                        <div class="mb-2">
                            <textarea class="form-control form-control-sm" id="lolAiLeadMessage" rows="2" placeholder="Message (optional)" maxlength="500"></textarea>
                        </div>
                        <div class="d-flex gap-2">
                            <button type="button" class="btn btn-sm btn-outline-secondary" id="lolAiLeadCancel">Cancel</button>
                            <button type="button" class="btn btn-sm btn-primary" id="lolAiLeadSubmit">
                                <span id="lolAiLeadSpinner" class="spinner-border spinner-border-sm me-1" role="status" style="display:none;"></span>Send
                            </button>
                        </div>
                    </div>
                    @endif
                    ...
                    <div id="lolAiResult" style="display:none;margin-top:1rem;"></div>
                </div>
                <div class="modal-footer border-0 pb-4">
                    <button type="button" class="btn btn-outline-secondary" data-bs-dismiss="modal">Cancel</button>
                    <button type="button" class="btn btn-primary" id="lolAiSubmitBtn">
                        <span id="lolAiSubmitSpinner" class="spinner-border spinner-border-sm me-1" role="status" style="display:none;"></span>
                        <i class="fa-solid fa-robot me-1" id="lolAiSubmitIcon"></i><span id="lolAiSubmitLabel">Ask AI</span>
                    </button>
                </div>
            </div>
        </div>
...
(function () {
    ...
    (function () {
        ...
        (function () {
            ...
            var resultDiv = document.getElementById('lolAiResult');
            var convDiv   = document.getElementById('lolAiConversation');
            var spinner   = document.getElementById('lolAiSubmitSpinner');
            var icon      = document.getElementById('lolAiSubmitIcon');
            var label     = document.getElementById('lolAiSubmitLabel');
            var modalEl   = document.getElementById('lolAiModal');
            ...
            var v2ListingType = 'landlord_offer';
            ...
            function renderResult(data) {
                ...
                if (status === 'ready' && data.answer) {
                    ...
                    html += '<div style="font-size:.8rem;font-weight:700;color:#0f766e;margin-bottom:.35rem;"><i class="fa-solid fa-robot me-1"></i>AI Answer</div>';
                    ...
                    html += '<div style="font-size:.875rem;color:#1e293b;">' + escHtml(_v1Ans) + '</div>';
                    html += '</div>';
                    if (data.disclosures) {
            ...
            function resetResult() { if (resultDiv) { resultDiv.style.display = 'none'; resultDiv.innerHTML = ''; } }
            function restoreHistory() {
                if (!convDiv) return;
                var hist = getHistory(); if (!hist.length) { convDiv.style.display = 'none'; convDiv.innerHTML = ''; return; }
                var html = ''; hist.forEach(function(item) { html += item.html || ''; });
                convDiv.innerHTML = html; convDiv.style.display = ''; convDiv.scrollTop = convDiv.scrollHeight;
            }
            function userBubble(q) {
                return '<div style="display:flex;justify-content:flex-end;margin-bottom:.4rem;"><div style="background:#2563eb;color:#fff;border-radius:.75rem .75rem .1rem .75rem;padding:.5rem .8rem;font-size:.84rem;max-width:88%;word-break:break-word;">' + escHtml(q) + '</div></div>';
            }
            function assistantBubble(data, q) {
                var status = data.status || 'failed', answer = data.answer || '', isEsc = false;
                if (status === 'unsupported') { answer = 'I may need the agent to confirm that. Would you like me to send them your question?'; isEsc = true; }
                if (data.escalate) { if (data.escalation_prompt) answer = data.escalation_prompt; isEsc = true; }
                var bg = '#f0fdfa', bd = '#99f6e4', hc = '#0f766e';
                ...
                var html = '<div style="margin-bottom:.75rem;"><div style="background:' + bg + ';border:1px solid ' + bd + ';border-radius:.1rem .75rem .75rem .75rem;padding:.65rem .85rem;">';
                html += '<div style="font-size:.72rem;font-weight:700;color:' + hc + ';margin-bottom:.3rem;"><i class="fa-solid fa-robot me-1"></i>AI</div>';
                if (status === 'blocked' && data.refusal_message) { html += '<div style="font-size:.875rem;color:#1e293b;">' + escHtml(data.refusal_message) + '</div>'; }
                else if (!answer) { html += '<div style="font-size:.875rem;color:#be123c;">Ask AI could not generate a response right now. Please try again later.</div>'; }
                else { html += '<div style="font-size:.875rem;color:#1e293b;white-space:pre-wrap;">' + escHtml(answer) + '</div>'; }
                html += '</div>';
                if (Array.isArray(data.actions) && data.actions.length > 0) {
                    html += '<div style="display:flex;gap:.4rem;flex-wrap:wrap;margin-top:.4rem;">';
                    data.actions.forEach(function(a) {
                        if (a.type === 'link' && a.href) { html += '<a href="' + escHtml(a.href) + '" style="display:inline-flex;align-items:center;gap:4px;font-size:.77rem;font-weight:600;padding:.28rem .62rem;border-radius:.45rem;border:1px solid #2563eb;color:#2563eb;background:#eff6ff;text-decoration:none;">' + (a.icon ? '<i class="fa-solid ' + escHtml(a.icon) + '"></i>' : '') + escHtml(a.label || a.key || 'View') + '</a>'; }
                        else { html += '<button type="button" class="agentai-v2-action-btn" data-action-key="' + escHtml(a.key || '') + '" style="display:inline-flex;align-items:center;gap:4px;font-size:.77rem;font-weight:600;padding:.28rem .62rem;border-radius:.45rem;border:1px solid #10b981;color:#065f46;background:#ecfdf5;cursor:pointer;">' + (a.icon ? '<i class="fa-solid ' + escHtml(a.icon) + '"></i>' : '') + escHtml(a.label || a.key || 'Action') + '</button>'; }
                    });
                    html += '</div>';
                }
                if (isEsc) { html += '<div style="margin-top:.4rem;display:flex;gap:.4rem;flex-wrap:wrap;"><button type="button" class="agentai-v2-escalate-yes" data-question="' + escHtml(q) + '" style="display:inline-flex;align-items:center;gap:4px;font-size:.77rem;font-weight:600;padding:.28rem .62rem;border-radius:.45rem;border:1px solid #2563eb;color:#2563eb;background:#eff6ff;cursor:pointer;"><i class="fa-solid fa-paper-plane"></i>Yes, send my question</button></div>'; }
            ...
            function appendTurn(q, data) {
                var uHtml = userBubble(q), aHtml = assistantBubble(data, q);
                var hist = getHistory(); hist.push({ html: uHtml }); hist.push({ html: aHtml });
            ...
            function submitV1(q) {
                ...
                fetch('/ask-ai/listing-question', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': csrfToken, 'Accept': 'application/json' },
                    body: JSON.stringify({ listing_type: 'landlord', listing_id: listingId, question: q })
                })
                .then(function(r) { return r.json(); })
            ...
            if (resultDiv) {
                resultDiv.addEventListener('click', function(e) {
                    ...
                    if (ctaBtn) {
                        ...
                        var qModalEl = document.getElementById('lolQuestionModal');
            ...
            if (convDiv) {
                convDiv.addEventListener('click', function(e) {
                    ...
                    if (escBtn) {
                        ...
                        .then(function(r) { return r.json(); })
                        .then(function(d) { var msg = (d && d.message) ? d.message : 'Your question has been sent to the agent.'; escBtn.outerHTML = '<span style="font-size:.78rem;color:#0f766e;"><i class="fa-solid fa-check-circle me-1"></i>' + escHtml(msg) + '</span>'; })
                    ...
                    if (actBtn) {
                        ...
                        var leadForm  = document.getElementById('lolAiLeadForm');
                        var leadTitle = document.getElementById('lolAiLeadTitle');
                        var leadAK    = document.getElementById('lolAiLeadActionKey');
                        var leadMsg   = document.getElementById('lolAiLeadMessage');
                        if (leadForm) {
            ...
            (function() {
                var leadForm   = document.getElementById('lolAiLeadForm');
                var leadSubmit = document.getElementById('lolAiLeadSubmit');
                var leadCancel = document.getElementById('lolAiLeadCancel');
                ...
                leadSubmit.addEventListener('click', function() {
                    var name  = ((document.getElementById('lolAiLeadName')  || {}).value || '').trim();
                    var email = ((document.getElementById('lolAiLeadEmail') || {}).value || '').trim();
                    var msg   = ((document.getElementById('lolAiLeadMessage') || {}).value || '').trim();
                    var ak    = ((document.getElementById('lolAiLeadActionKey') || {}).value || '');
                    if (!name || !email) return;
                    var spinner = document.getElementById('lolAiLeadSpinner');
            ...
            (function() {
                ...
                var chipsWrap = document.getElementById('lolAiSuggestions');
                if (!chipsWrap) return;
                ...
                chipsWrap.querySelectorAll('.ask-ai-chip').forEach(function(btn) {
                    btn.addEventListener('click', function() { var q = btn.getAttribute('data-question'); if (q && textarea) { textarea.value = q; textarea.focus(); } markUsed(q); btn.style.display = 'none'; });
            ...
            if (modalEl) {
                modalEl.addEventListener('show.bs.modal', function() {
                    setLoading(false);
                    if (useV2 && v2AgentId && v2Scope) { initV2(function(ok) { if (ok) restoreHistory(); }); }
                });
                modalEl.addEventListener('hidden.bs.modal', function() {
                    if (textarea) textarea.value = '';
                    resetResult();
                    var lf = document.getElementById('lolAiLeadForm'); if (lf) lf.style.display = 'none';
                });
            }
            if (submitBtn) {
                submitBtn.addEventListener('click', function() {
                    ...
                    lastQuestion = q;
                    setLoading(true);
                    if (useV2 && v2Active) { submitV2(q); }
                    else if (useV2 && v2AgentId && v2Scope && !v2Failed) { initV2(function(ok) { if (ok) { submitV2(q); } else { submitV1(q); } }); }
                    else { submitV1(q); }
                });
            }
        }());
