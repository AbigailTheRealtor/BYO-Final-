...
/* ---- Ask AI modal submit button — override Tailwind preflight ----
   ...
   ----------------------------------------------------------------- */
#bolAiModal .modal-footer .btn-primary {
    background-color: #2563eb !important;
    border-color: #2563eb !important;
    color: #ffffff !important;
}
#bolAiModal .modal-footer .btn-primary:hover,
#bolAiModal .modal-footer .btn-primary:focus {
...
<div class="container py-4 bol-view-page">
    ...
    @php
        ...
        // RESTORED in Stage 1: Buyer listings now create their OfferAuction and stamp
            ...
            <div class="modal-content" style="border-radius:.85rem;overflow:hidden;border:none;">
                ...
                <div class="modal-body p-4">
                    ...
                    <div id="bolAiSuggestions"
                        ...
                        @foreach($__bolCategorized as $__catKey => $__catItems)
                        <div class="mb-2">
                    ...
                    <div id="bolAiConversation" style="display:none;max-height:260px;overflow-y:auto;margin-bottom:.75rem;padding:.6rem .75rem;background:#f8fafc;border:1px solid #e2e8f0;border-radius:.5rem;scroll-behavior:smooth;"></div>
                    {{-- V2: inline lead-capture form (shown when CTA action button clicked) --}}
                    <div id="bolAiLeadForm" style="display:none;margin-top:.5rem;margin-bottom:.75rem;background:#f0f9ff;border:1px solid #bae6fd;border-radius:.5rem;padding:.85rem 1rem;">
                        <div style="font-size:.78rem;font-weight:700;color:#0369a1;margin-bottom:.5rem;">
                            <i class="fa-solid fa-paper-plane me-1" aria-hidden="true"></i><span id="bolAiLeadTitle">Contact the Agent</span>
                        </div>
                        <input type="hidden" id="bolAiLeadActionKey">
                        <div class="mb-2">
                            <input type="text" class="form-control form-control-sm" id="bolAiLeadName" placeholder="Your name" maxlength="100" autocomplete="name">
                        </div>
                        <div class="mb-2">
                            <input type="email" class="form-control form-control-sm" id="bolAiLeadEmail" placeholder="Your email" maxlength="200" autocomplete="email">
                        </div>
                        <div class="mb-2">
                            <textarea class="form-control form-control-sm" id="bolAiLeadMessage" rows="2" placeholder="Message (optional)" maxlength="500"></textarea>
                        </div>
                        <div class="d-flex gap-2">
                            <button type="button" class="btn btn-sm btn-outline-secondary" id="bolAiLeadCancel">Cancel</button>
                            <button type="button" class="btn btn-sm btn-primary" id="bolAiLeadSubmit">
                                <span id="bolAiLeadSpinner" class="spinner-border spinner-border-sm me-1" role="status" style="display:none;"></span>Send
                            </button>
                        </div>
                    </div>
                    @endif
...
(function () {
    ...
    (function () {
        ...
        var convDiv    = document.getElementById('bolAiConversation');
        ...
        var v2ListingType = 'buyer_criteria';
        ...
        function submitV1(q) {
            ...
            fetch('/ask-ai/listing-question', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': csrfToken, 'Accept': 'application/json' },
                body: JSON.stringify({ listing_type: 'buyer', listing_id: listingId, question: q })
            })
            .then(function(r) { return r.json(); })
        ...
        if (convDiv) {
            convDiv.addEventListener('click', function(e) {
                ...
                if (actBtn) {
                    ...
                    var leadForm  = document.getElementById('bolAiLeadForm');
                    var leadTitle = document.getElementById('bolAiLeadTitle');
                    var leadAK    = document.getElementById('bolAiLeadActionKey');
                    var leadMsg   = document.getElementById('bolAiLeadMessage');
                    if (leadForm) {
        ...
        (function() {
            var leadForm   = document.getElementById('bolAiLeadForm');
            var leadSubmit = document.getElementById('bolAiLeadSubmit');
            var leadCancel = document.getElementById('bolAiLeadCancel');
            ...
            leadSubmit.addEventListener('click', function() {
                var name  = ((document.getElementById('bolAiLeadName')  || {}).value || '').trim();
                var email = ((document.getElementById('bolAiLeadEmail') || {}).value || '').trim();
                var msg   = ((document.getElementById('bolAiLeadMessage') || {}).value || '').trim();
                var ak    = ((document.getElementById('bolAiLeadActionKey') || {}).value || '');
                if (!name || !email) return;
                var spinner = document.getElementById('bolAiLeadSpinner');
        ...
        if (modalEl) {
            ...
            modalEl.addEventListener('hidden.bs.modal', function() {
                ...
                resetResult();
                var lf = document.getElementById('bolAiLeadForm'); if (lf) lf.style.display = 'none';
            });
        }
