        ...
        <div class="modal-content" style="border-radius:.85rem;overflow:hidden;border:none;">
            ...
            <div class="modal-body p-4">
                ...
                <div id="tclAiSuggestions"
                    ...
                    @foreach($__tclCategorized as $__catKey => $__catItems)
                    <div class="mb-2">
                ...
                <div id="tclAiConversation" style="display:none;max-height:260px;overflow-y:auto;margin-bottom:.75rem;padding:.6rem .75rem;background:#f8fafc;border:1px solid #e2e8f0;border-radius:.5rem;scroll-behavior:smooth;"></div>
                {{-- V2: inline lead-capture form (shown when CTA action button clicked) --}}
                <div id="tclAiLeadForm" style="display:none;margin-top:.5rem;margin-bottom:.75rem;background:#f0f9ff;border:1px solid #bae6fd;border-radius:.5rem;padding:.85rem 1rem;">
                    <div style="font-size:.78rem;font-weight:700;color:#0369a1;margin-bottom:.5rem;">
                        <i class="fa-solid fa-paper-plane me-1" aria-hidden="true"></i><span id="tclAiLeadTitle">Contact the Agent</span>
                    </div>
                    <input type="hidden" id="tclAiLeadActionKey">
                    <div class="mb-2">
                        <input type="text" class="form-control form-control-sm" id="tclAiLeadName" placeholder="Your name" maxlength="100" autocomplete="name">
                    </div>
                    <div class="mb-2">
                        <input type="email" class="form-control form-control-sm" id="tclAiLeadEmail" placeholder="Your email" maxlength="200" autocomplete="email">
                    </div>
                    <div class="mb-2">
                        <textarea class="form-control form-control-sm" id="tclAiLeadMessage" rows="2" placeholder="Message (optional)" maxlength="500"></textarea>
                    </div>
                    <div class="d-flex gap-2">
                        <button type="button" class="btn btn-sm btn-outline-secondary" id="tclAiLeadCancel">Cancel</button>
                        <button type="button" class="btn btn-sm btn-primary" id="tclAiLeadSubmit">
                            <span id="tclAiLeadSpinner" class="spinner-border spinner-border-sm me-1" role="status" style="display:none;"></span>Send
                        </button>
                    </div>
                </div>
                @endif
...
(function () {
    ...
    (function () {
        ...
        (function () {
            ...
            var convDiv   = document.getElementById('tclAiConversation');
            ...
            var v2ListingType = 'tenant_criteria';
            ...
            function assistantBubble(data, q) {
                ...
                var bg = '#f0fdfa', bd = '#99f6e4', hc = '#0d9488';
            ...
            function submitV1(q) {
                ...
                fetch('/ask-ai/listing-question', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': csrfToken, 'Accept': 'application/json' },
                    body: JSON.stringify({ listing_type: 'tenant', listing_id: listingId, question: q })
                })
                .then(function(r) { return r.json(); })
            ...
            if (resultDiv) {
                resultDiv.addEventListener('click', function(e) {
                    ...
                    if (ctaBtn) {
                        ...
                        var qModalEl = document.getElementById('tclQuestionModal');
            ...
            if (convDiv) {
                convDiv.addEventListener('click', function(e) {
                    ...
                    if (escBtn) {
                        ...
                        .then(function(r) { return r.json(); })
                        .then(function(d) { var msg = (d && d.message) ? d.message : 'Your question has been sent to the agent.'; escBtn.outerHTML = '<span style="font-size:.78rem;color:#0d9488;"><i class="fa-solid fa-check-circle me-1"></i>' + escHtml(msg) + '</span>'; })
                    ...
                    if (actBtn) {
                        ...
                        var leadForm  = document.getElementById('tclAiLeadForm');
                        var leadTitle = document.getElementById('tclAiLeadTitle');
                        var leadAK    = document.getElementById('tclAiLeadActionKey');
                        var leadMsg   = document.getElementById('tclAiLeadMessage');
                        if (leadForm) {
            ...
            (function() {
                var leadForm   = document.getElementById('tclAiLeadForm');
                var leadSubmit = document.getElementById('tclAiLeadSubmit');
                var leadCancel = document.getElementById('tclAiLeadCancel');
                ...
                leadSubmit.addEventListener('click', function() {
                    var name  = ((document.getElementById('tclAiLeadName')  || {}).value || '').trim();
                    var email = ((document.getElementById('tclAiLeadEmail') || {}).value || '').trim();
                    var msg   = ((document.getElementById('tclAiLeadMessage') || {}).value || '').trim();
                    var ak    = ((document.getElementById('tclAiLeadActionKey') || {}).value || '');
                    if (!name || !email) return;
                    var spinner = document.getElementById('tclAiLeadSpinner');
            ...
            if (modalEl) {
                ...
                modalEl.addEventListener('hidden.bs.modal', function() {
                    ...
                    resetResult();
                    var lf = document.getElementById('tclAiLeadForm'); if (lf) lf.style.display = 'none';
                });
            }
            if (submitBtn) {
