...
<div class="container listingDescription tcl-view-page">
    ...
    @php
        ...
        $hasPetsSection = count($petsDisplay) || $ifFilled($str('number_of_pets'))
            ...
            || $ifFilled($str('garage_needed')) || $ifFilled($str('carport_needed')) || $ifFilled($str('has_breed_restrictions'))
    ...
    @php ob_start(); @endphp
            <div class="row">
                ...
                <div class="col-md-12">
                    ...
                    {{-- Carport and garage are two questions on the tenant form and are two facts
                         here. They used to share one "Parking Needed" row that printed the first
                         non-empty of carport / garage / parking, so a "No" carport hid a "Yes"
                         garage. Each now has its own row; the legacy parking_needed keeps its own. --}}
                    {!! $row('Carport Needed', $resolveOtherField($str('carport_needed'), $str('other_carport_needed'))) !!}
                    {!! $row('Garage Needed', $resolveOtherField($str('garage_needed'), $str('other_garage_needed'))) !!}
                    {!! $row('Parking Needed', $str('parking_needed')) !!}
                </div>
            </div>
    @php $__olSectionBody = trim(ob_get_clean()); @endphp
    {{-- A section renders only when its rows produced something: a heading over nothing is the old empty-section bug. --}}
...
@include('offer-listing.partials._ask-ai-question-modal', [
    ...
    'prefix'        => 'tcl',
    'modalId'       => 'tclAiModal',
    'title'         => 'Ask AI About This Tenant Listing',
    'questions'     => $propertyQuestions ?? [],
    'meta'          => $meta ?? [],
    'viewerIsOwner' => $askAiViewerIsOwner ?? false,
