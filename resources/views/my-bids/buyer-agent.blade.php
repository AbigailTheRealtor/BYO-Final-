                ...
                <div class="card p-3 mb-3">
                    <div class="row myBidsDetails align-items-center">
                        <div class="col-12 col-md-4 col-lg-4">
                            ...
                            <small class="text-muted">Listing ID: {{ $auction->listing_id ?? 'BAA-'.$auction->id }}</small>
                        </div>
                        ...
                        <div class="col-12 col-md-6 col-lg-6 text-end">
                            ...
                            @elseif($bidStatus === 'Countered')
                                <a href="{{ route('agent.buyer.agent.auction.bid.view-counter', $bid->id) }}" class="btn btn-sm btn-warning text-dark">
                                    View Counter
                                </a>
                            @elseif($bidStatus === 'Rejected')
                                <a href="{{ route('buyer.view-auction', $auction->id) }}" class="btn btn-sm btn-outline-secondary">
                                    View Listing
                                </a>
                            @elseif($bidStatus === 'Active')
                                ...
                                @endif
                                <a href="{{ route('agent.buyer.agent.auction.bid', $auction->id) }}?edit={{ $bid->id }}" class="btn btn-sm btn-primary me-2">
                    ...
                    @if(!empty($bid->get->maxBudget))
                        <div class="fw-bold mt-2">
                            {{ $bid->get->maxBudget }}
                            <span class="text-sm-end opacity-5 badge bg-secondary">Max Budget</span>
                        </div>
                    @endif
                </div>
            @endforeach
