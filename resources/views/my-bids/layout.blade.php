@extends('layouts.main')
@push('styles')
<style>
    #myTab .nav-link {
    font-size: 14px;
    padding: 5px;
    border: 1px solid rgba(0,0,0,0.2);
    margin: 1px;
    border-radius: 5px;
}
</style>
@endpush
@section('content')
    <div class="mainDashboard">
        <div class="container">
            @include('layouts.partials.dashboard_user_section')
            <div class="dashboardContentDetails mt-3">
                <div class="card">
                    <div class="row">
                        @include('layouts.partials.sidenav')
                        <div class="rightCol col-sm-12 col-md-9 col-lg-9">
                            @if(auth()->check() && auth()->user()->user_type == 'tenant')
                            <h1>Agent Bids</h1>
                            @else
                            <h1>My Bids</h1>
                            <div class="p-2">
                                <ul class="nav nav-tabs pb-2" id="myTab" role="tablist">
                                    @php
                                        $id = auth()->id();
                                    @endphp
                                    @if(auth()->check())
                                    <li class="nav-item" role="presentation">
                                      <a href="{{route('myBids', 'seller-property')}}" class="nav-link {{ ($type=='seller-property')?"active":"" }}">Property Listing (Sale)</a>
                                    </li>
                                    <li class="nav-item" role="presentation">
                                        <a href="{{route('myBids', 'landlord-property')}}" class="nav-link {{ ($type=='landlord-property')?"active":"" }}">Property Listing (Rental)</a>
                                    </li>
                                    <li class="nav-item" role="presentation">
                                        <a href="{{route('myBids', 'buyer-criteria')}}" class="nav-link {{ ($type=='buyer-criteria')?"active":"" }}">Buyer's Criteria</a>
                                    </li>
                                    <li class="nav-item" role="presentation">
                                        <a href="{{route('myBids', 'tenant-criteria')}}" class="nav-link {{ ($type=='tenant-criteria')?"active":"" }}">Tenant's Criteria</a>
                                    </li>
                                    <li class="nav-item" role="presentation">
                                        <a href="{{route('myBids', 'agent-service')}}" class="nav-link {{ ($type=='agent-service')?"active":"" }}">Agent Service Needed</a>
                                    </li>
                                    <li class="nav-item" role="presentation">
                                        <a href="{{route('myBids', 'buyer-agent')}}" class="nav-link {{ ($type=='buyer-agent')?"active":"" }}">Hiring Buyer's Agent</a>
                                    </li>
                                    <li class="nav-item" role="presentation">
                                        <a href="{{route('myBids', 'seller-agent')}}" class="nav-link {{ ($type=='seller-agent')?"active":"" }}">Hiring Seller's Agent</a>
                                    </li>
                                    <li class="nav-item" role="presentation">
                                        <a href="{{route('myBids', 'landlord-agent')}}" class="nav-link {{ ($type=='landlord-agent')?"active":"" }}">Hiring Landlord's Agent</a>
                                    </li>
                                    <li class="nav-item" role="presentation">
                                        <a href="{{route('myBids', 'tenant-agent')}}" class="nav-link {{ ($type=='tenant-agent')?"active":"" }}">Hiring Tenant's Agent</a>
                                    </li>
                                    <li class="nav-item" role="presentation">
                                        <a href="{{route('myBids', 'agent-bids')}}" class="nav-link {{ ($type=='agent-bids')?"active":"" }}" style="background-color: {{ ($type=='agent-bids') ? '#049399' : 'transparent' }}; color: {{ ($type=='agent-bids') ? '#fff' : '#049399' }}; border-color: #049399;">Tenant's Agent Bids</a>
                                    </li>
                                    <li class="nav-item" role="presentation">
                                        <a href="{{route('myBids', 'hire-landlord-agent-bids')}}" class="nav-link {{ ($type=='hire-landlord-agent-bids')?"active":"" }}" style="background-color: {{ ($type=='hire-landlord-agent-bids') ? '#049399' : 'transparent' }}; color: {{ ($type=='hire-landlord-agent-bids') ? '#fff' : '#049399' }}; border-color: #049399;">Landlord's Agent Bids</a>
                                    </li>
                                    <li class="nav-item" role="presentation">
                                        <a href="{{route('myBids', 'hire-buyer-agent-bids')}}" class="nav-link {{ ($type=='hire-buyer-agent-bids')?"active":"" }}" style="background-color: {{ ($type=='hire-buyer-agent-bids') ? '#049399' : 'transparent' }}; color: {{ ($type=='hire-buyer-agent-bids') ? '#fff' : '#049399' }}; border-color: #049399;">Buyer's Agent Bids</a>
                                    </li>
                                    <li class="nav-item" role="presentation">
                                        <a href="{{route('myBids', 'hire-seller-agent-bids')}}" class="nav-link {{ ($type=='hire-seller-agent-bids')?"active":"" }}" style="background-color: {{ ($type=='hire-seller-agent-bids') ? '#049399' : 'transparent' }}; color: {{ ($type=='hire-seller-agent-bids') ? '#fff' : '#049399' }}; border-color: #049399;">Seller's Agent Bids</a>
                                    </li>
                                    @endif
                                </ul>
                            </div>
                            @endif

                            @yield('bids-content')
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
@endsection
@push('scripts')
@endpush
