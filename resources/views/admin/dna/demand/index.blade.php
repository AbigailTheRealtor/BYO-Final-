...
<div class="card">
    <div class="card-header d-flex align-items-center justify-content-between flex-wrap gap-2">
        <h5 class="mb-0">DNA Inspector — Demand DNA Profiles</h5>
        <span class="text-muted small">Read-only diagnostic view &bull; <code>buyer_tenant_dna_profiles</code></span>
    </div>

    <div class="card-body border-bottom pb-3">
        <form method="GET" action="{{ route('admin.dna.demand.index') }}" class="row g-2 align-items-end">
            <div class="col-md-2">
                <label class="form-label small mb-1">Listing Type</label>
                <input type="text" name="listing_type" class="form-control form-control-sm" value="{{ $filters['listing_type'] ?? '' }}" placeholder="e.g. buyer">
            </div>
            <div class="col-md-2">
            ...
            <table class="table table-sm table-bordered table-hover mb-0" style="font-size:.82rem;">
                <thead class="thead-light">
                    <tr>
                        ...
                        <th>Version Count</th>
                        <th>Status</th>
                        <th>Preference Completeness</th>
                        <th>Conflict Dimensions (metadata)</th>
                        <th>Computed At</th>
                        <th></th>
                    </tr>
                </thead>
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{ $row->preference_completeness ?? 'No data' }}</td>
                        <td>
                            @if(!is_null($row->deal_breaker_flags))
                                <span class="text-dark">Conflict Dimensions (metadata)</span>
                            @else
                                <span class="text-muted">Conflict Dimensions — Not yet populated</span>
                            @endif
                        </td>
                        <td>{{ $row->computed_at ? $row->computed_at->format('Y-m-d H:i') : '—' }}</td>
                        <td><a href="{{ route('admin.dna.demand.show', $row->id) }}" class="btn btn-xs btn-outline-secondary">View</a></td>
                    </tr>
                    @empty
