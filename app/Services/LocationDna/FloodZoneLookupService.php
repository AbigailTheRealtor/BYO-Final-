...
class FloodZoneLookupService
{
    ...
    public function __construct(private FloodZoneAdapterInterface $adapter)
    {
    }

    /**
     * Resolve FEMA flood zone polygons for the active location boundary.
     *
     ...
     * then delegates to the FloodZoneAdapterInterface implementation.
     *
     ...
     * @return array  ['flood_zones' => [...], 'available' => bool]
     *                flood_zones: array of ['zone_designation' => string, 'rings' => array]
    ...
    {
        $unavailable = ['flood_zones' => [], 'available' => false];
        ...
        // there is nothing to derive a bounding box from — skip the FEMA call.
        ...
        // Area guard: skip the FEMA call for unreasonably large areas.
        $area = ($bbox[2] - $bbox[0]) * ($bbox[3] - $bbox[1]);
        $maxArea = (float) config('location_dna.flood_zone_max_area_sq_degrees', 2.0);

        if ($area > $maxArea) {
            Log::warning('FloodZoneLookupService: skipping FEMA lookup — bounding area exceeds threshold', [
        ...
        $floodZones = $this->adapter->lookup($bbox);
    ...
    private function deriveBbox(array $boundaryData, array $polygons, array $radii): ?array
    {
        ...
        // Small padding (0.01°) to ensure boundary-edge zones are captured
        return [
            round($minLng - 0.01, 6),
            round($minLat - 0.01, 6),
            round($maxLng + 0.01, 6),
