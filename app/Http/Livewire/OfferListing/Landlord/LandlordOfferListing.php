...
class LandlordOfferListing extends Component
{
    ...
    public function mount($listingId = null)
    {
        ...
        // Owner + product scoped. The old query also accepted `is_draft IS NULL`,
        // which never matched: the column is NOT NULL DEFAULT false on all four
        // tables, so that arm was unreachable and dropping it changes nothing.
    ...
    public function loadDraft($listingId)
    {
        ...
        if ($auction) {
            ...
            $this->heating_fuel = $this->ensureArray($auction->get->heating_fuel ?? []);
            $this->other_heating_fuel = $auction->get->other_heating_fuel ?? '';
            $this->air_conditioning = $this->ensureArray($auction->get->air_conditioning ?? []);
            $this->other_air_conditioning = $auction->get->other_air_conditioning ?? '';
            $this->water = $this->ensureArray($auction->get->water ?? []);
            $this->other_water = $auction->get->other_water ?? '';
            $this->sewer = $this->ensureArray($auction->get->sewer ?? []);
            $this->other_sewer = $auction->get->other_sewer ?? '';
            $this->property_utilities = $this->ensureArray($auction->get->property_utilities ?? []);
            $this->other_property_utilities = $auction->get->other_property_utilities ?? '';
            $this->laundry_features = $this->ensureArray($auction->get->laundry_features ?? []);
            $this->other_laundry_features = $auction->get->other_laundry_features ?? '';
            $this->floor_covering = $this->ensureArray($auction->get->floor_covering ?? []);
            $this->other_floor_covering = $auction->get->other_floor_covering ?? '';
            $this->security_features = $this->ensureArray($auction->get->security_features ?? []);
            ...
            $this->road_surface_type = $this->ensureArray($auction->get->road_surface_type ?? []);
            $this->other_road_surface_type = $auction->get->other_road_surface_type ?? '';
            $this->electrical_service = $this->ensureArray($auction->get->electrical_service ?? []);
            $this->other_electrical_service = $auction->get->other_electrical_service ?? '';
            $this->ceiling_height = $auction->get->ceiling_height ?? '';
            $this->building_features = $this->ensureArray($auction->get->building_features ?? []);
            ...
            $this->space_type = $this->ensureArray($auction->get->space_type ?? []);
            $this->other_space_type = $auction->get->other_space_type ?? '';
            $this->space_classification = $this->ensureArray($auction->get->space_classification ?? []);
