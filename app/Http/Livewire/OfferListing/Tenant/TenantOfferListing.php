...
class TenantOfferListing extends Component
{
     ...
     *
     * This class is a four-role switch like the catch-all Hire component, so it cannot name a
     * single workflow: `store()` maps `user_type` to four different auction models and the root
     * blade picks a different property tab per role. Only the tenant role has a Create Tenant
     * geography surface, so every other role resolves to NULL and is disabled by the TYPE of the
     * argument rather than by the contents of a config file.
     *
     * `create_tenant` is deliberately its own key rather than reusing `hire_tenant`: the two are
     * separate rollout steps and separate record families, and one scope-list entry must never
     * switch on a workflow nobody verified.
     */
    ...
    public function mount($user_type = null, $listingId = null)
    {
        ...
        // Phase 1c — decide whether the cascade runs, now that the role is known. Every role but
        // tenant resolves to a null workflow and is disabled outright.
    ...
    {
        return view('livewire.offer-listing.tenant.offer-tenant-listing')->extends('layouts.main')->section('content'); // Define the section
    }
    ...
    protected function buildDraftPayload(): array
    {
        ...
        if (is_array($this->condition_prop_buyer)) {
            $condPropBuyer = array_values(array_unique(array_map('trim', $this->condition_prop_buyer)));
        } else {
            $fromJsonCpb = json_decode($this->condition_prop_buyer_json, true);
            $condPropBuyer = (is_array($fromJsonCpb) && !empty($fromJsonCpb))
        ...
        $exchangeItemVal = is_array($this->exchange_item)
            ? array_values(array_filter((array) $this->exchange_item))
            : (is_string($this->exchange_item) && trim($this->exchange_item) !== '' ? [$this->exchange_item] : []);

        if ($this->user_type === 'seller' || $this->user_type === 'landlord') {
            $propertyItems = is_string($this->property_items) ? $this->property_items : '';
        } else {
            $propertyItems = json_encode(
                is_string($this->property_items)
                    ? (json_decode($this->property_items, true) ?? [])
                    : (array) $this->property_items
            );
        }
        ...
        $data = [
            ...
            'leasing_spaces_tenant' => json_encode($this->leasing_spaces_tenant),
            'tenant_pays'         => json_encode($this->tenant_pays),
            'leasing_space'       => $this->leasing_space,
            'restrictions'        => $this->restrictions,
            'common_areas_access' => $this->common_areas_access,
            'maintenance_by'      => $this->maintenance_by,
            'maintenance_response_time' => $this->maintenance_response_time,
            'storage_space'       => $this->storage_space,
            'included_storage_space_com_entire' => $this->included_storage_space_com_entire,
            'storage_space_com_entire' => $this->storage_space_com_entire,
            'utilities'           => $this->utilities,
            'common_areas_cleaning' => $this->common_areas_cleaning,
            'included_storage_space_com_single' => $this->included_storage_space_com_single,
            'storage_space_com_single' => $this->storage_space_com_single,
            'included_storage_space_res_both' => $this->included_storage_space_res_both,
            'storage_space_res_both' => $this->storage_space_res_both,
            'included_storage_space_res_single' => $this->included_storage_space_res_single,
            'storage_space_res_single' => $this->storage_space_res_single,
            'bathroom_facilities' => $this->bathroom_facilities,
            'room_size'           => $this->room_size,
            'building_hours'      => $this->building_hours,
            'access_24_7'         => $this->access_24_7,
            'zoning_allows'       => $this->zoning_allows,
            'shared_amenities'    => $this->shared_amenities,
            'space_features'      => $this->space_features,
            'neighboring_tenants' => $this->neighboring_tenants,
            'guests_allowed'      => $this->guests_allowed,
            'condition_prop'      => $this->condition_prop,
            'business_type'       => $this->business_type,
            'business_type_selected' => $this->business_type_selected,
            ...
            'total_acreage'       => $this->stripCommas($this->total_acreage),
            'other_tenant_pays'   => $this->other_tenant_pays,
            'tenant_require'      => json_encode($this->tenant_require),
            'desired_lease_length' => json_encode($this->desired_lease_length),
            'other_lease_term'    => $this->other_lease_term,
            'rent_includes'       => json_encode($this->rent_includes),
            'other_rent_include'  => $this->other_rent_include,
            'owner_pays'          => json_encode($this->owner_pays),
            'other_owner_pays'    => $this->other_owner_pays,
            'owner_pays_other'    => $this->owner_pays_other,
            'terms_of_lease'      => json_encode($this->terms_of_lease),
            'custom_lease_term'   => $this->custom_lease_term,
            'garage_parking_spaces_option_buyer' => json_encode($this->garage_parking_spaces_option_buyer),
            ...
            'sqft_heated_source'  => $this->sqft_heated_source,
            ...
            'garage_parking_spaces_option' => json_encode($this->garage_parking_spaces_option),
            'other_parking_space_wrapper' => $this->other_parking_space_wrapper,
            'pool_needed'         => in_array($this->property_type ?? '', ['Residential Property', 'Residential', 'Income', 'Income Property']) ? $this->pool_needed : null,
            'pool_type'           => in_array($this->property_type ?? '', ['Residential Property', 'Residential', 'Income', 'Income Property']) ? json_encode($this->pool_type) : json_encode([]),
            ...
            'business_assets'     => json_encode(is_array($this->business_assets) ? $this->business_assets : []),
            'property_criteria'   => $this->property_criteria,
            'unit_size'           => $this->unit_size,
            'unit_size_other'     => $this->unit_size_other,
            'budget'              => $this->budget,
            'occupied_until'      => $this->occupied_until,
            ...
            'minimum_cap_rate'    => $this->minimum_cap_rate,
            'number_of_unit_type' => json_encode($this->number_of_unit_type),
            'unit_number'         => $this->unit_number,
            'unit_buildings'      => $this->unit_buildings,
            'unit_type_configurations' => json_encode(array_values($cleanedConfigs)),
            'screening_concerns'  => $this->screening_concerns,
            'screening_concerns_explanation' => $this->screening_concerns_explanation,
            ...
            'leasing_spaces'      => $this->leasing_spaces,
            'occupant_tenant'     => $this->occupant_tenant,
            ...
            'seller_down_payment_amount' => $this->stripCommas($this->seller_down_payment_amount),
            ...
            'max_assumable_rate'  => $this->max_assumable_rate,
            ...
            'lease_option_payment' => $this->lease_option_payment,
            ...
            'lease_purchase_payment' => $this->lease_purchase_payment,
            ...
            'crypto_percentage'   => $this->crypto_percentage,
            'cash_percentage_crypto' => $this->cash_percentage_crypto,
            ...
            'lease_option_fee_credit_percentage' => $this->lease_option_fee_credit_percentage,
            ...
            'lease_purchase_deposit' => $this->lease_purchase_deposit,
            ...
            'lease_fee_flat_type' => $this->lease_fee_flat_type,
            ...
            'lease_fee_percentage_monthly_number' => $this->lease_fee_percentage_monthly_number,
            'lease_fee_percentage_net' => $this->lease_fee_percentage_net,
            'lease_fee_flat_combo_net' => $this->stripCommas($this->lease_fee_flat_combo_net),
            'lease_fee_percentage_combo_net' => $this->lease_fee_percentage_combo_net,
            ...
            'interested_purchase_fee_type' => $this->interested_purchase_fee_type,
            'seller_leasing_fee_type' => $this->seller_leasing_fee_type,
            'seller_leasing_gross' => $this->seller_leasing_gross,
            'seller_leasing_gross_month_rent' => $this->seller_leasing_gross_month_rent,
            'seller_leasing_gross_rental' => $this->seller_leasing_gross_rental,
            'seller_broker_leasing_fee' => $this->seller_broker_leasing_fee,
            'seller_leasing_each_rental' => $this->seller_leasing_each_rental,
            'seller_leasing_gross_no_of_months' => $this->seller_leasing_gross_no_of_months,
            'purchase_fee_flat_commercial' => $this->stripCommas($this->purchase_fee_flat_commercial),
            'seller_leasing_gross_flat_combo' => $this->seller_leasing_gross_flat_combo,
            'seller_leasing_gross_percentage_combo' => $this->seller_leasing_gross_percentage_combo,
            'seller_leasing_gross_flat_net_combo' => $this->seller_leasing_gross_flat_net_combo,
            'seller_leasing_gross_percentage_net_combo' => $this->seller_leasing_gross_percentage_net_combo,
            'interested_in_selling' => $this->interested_in_selling,
            'interested_in_selling_type' => $this->interested_in_selling_type,
            'landlord_broker_purchase_price' => $this->stripCommas($this->landlord_broker_purchase_price),
            'landlord_broker_percentage_price' => $this->stripCommas($this->landlord_broker_percentage_price),
            'landlord_broker_dollar_price' => $this->stripCommas($this->landlord_broker_dollar_price),
            'landlord_broker_flate_fee' => $this->landlord_broker_flate_fee,
            'landlord_broker_other' => $this->landlord_broker_other,
            'broker_fee_timing'   => $this->broker_fee_timing,
            'broker_fee_days_from_rent' => $this->broker_fee_days_from_rent,
            'broker_fee_days_after_lease' => $this->broker_fee_days_after_lease,
            'broker_fee_days_after_rent' => $this->broker_fee_days_after_rent,
            'split_payment_due'   => $this->split_payment_due,
            'broker_fee_timing_other' => $this->broker_fee_timing_other,
            'broker_fee_days_after_due_event' => $this->broker_fee_days_after_due_event,
            'split_payment_due_other' => $this->split_payment_due_other,
            'renewal_fee_type'    => $this->renewal_fee_type,
            'renewal_fee_percentage' => $this->renewal_fee_percentage,
            'renewal_fee_lease_value' => $this->renewal_fee_lease_value,
            'renewal_fee_first_month' => $this->renewal_fee_first_month,
            'renewal_fee_flat_free' => $this->stripCommas($this->renewal_fee_flat_free),
            'renewal_fee_custom'  => $this->renewal_fee_custom,
            'renewal_fee_sales_tax_lease_value' => $this->renewal_fee_sales_tax_lease_value,
            'renewal_fee_sales_tax_flat_fee' => $this->renewal_fee_sales_tax_flat_fee,
            'renewal_fee_sales_tax_first_month' => $this->renewal_fee_sales_tax_first_month,
            'renewal_fee_no_of_months' => $this->renewal_fee_no_of_months,
            'expansion_commission_percentage' => $this->expansion_commission_percentage,
            'tenant_broker_commission_structure' => $this->tenant_broker_commission_structure,
            'tenant_broker_fee_structure' => $this->tenant_broker_fee_structure,
            'tenant_broker_percentage' => $this->tenant_broker_percentage,
            'tenant_broker_gross_lease' => $this->tenant_broker_gross_lease,
            'tenant_broker_first_month_rent' => $this->tenant_broker_first_month_rent,
            'tenant_broker_flat_fee' => $this->tenant_broker_flat_fee,
            'tenant_broker_other' => $this->tenant_broker_other,
            'seller_leasing_gross_sales_tax_option_gross' => $this->seller_leasing_gross_sales_tax_option_gross,
            'seller_leasing_gross_ross_percentage_rent' => $this->seller_leasing_gross_ross_percentage_rent,
            'seller_leasing_gross_sales_tax_flat_free_gross' => $this->seller_leasing_gross_sales_tax_flat_free_gross,
            'seller_leasing_gross_sales_tax_first_month' => $this->seller_leasing_gross_sales_tax_first_month,
            'seller_leasing_gross_percentage_no_of_months' => $this->seller_leasing_gross_percentage_no_of_months,
            'seller_leasing_gross_purchase_fee_flat_amount' => $this->stripCommas($this->seller_leasing_gross_purchase_fee_flat_amount),
            'seller_leasing_gross_purchase_fee_other' => $this->seller_leasing_gross_purchase_fee_other,
            'seller_leasing_gross_other' => $this->seller_leasing_gross_other,
            'interested_in_property_management' => $this->interested_in_property_management,
            'interested_in_property_management_fee' => $this->interested_in_property_management_fee,
            'interested_in_property_management_fee_gross_lease' => $this->interested_in_property_management_fee_gross_lease,
            'interested_in_property_management_fee_rental_periord' => $this->interested_in_property_management_fee_rental_periord,
            'interested_in_property_management_fee_flate_free' => $this->stripCommas($this->interested_in_property_management_fee_flate_free),
            'interested_in_property_management_fee_other' => $this->interested_in_property_management_fee_other,
            'purchase_fee_type'   => $this->purchase_fee_type,
            'purchase_fee_gross_rent' => $this->purchase_fee_gross_rent,
            'sales_tax_option_gross' => $this->sales_tax_option_gross,
            'sales_tax_option_flat' => $this->sales_tax_option_flat,
            'purchase_fee_net_aggregate' => $this->purchase_fee_net_aggregate,
            'purchase_fee_monthly_percentage' => $this->purchase_fee_monthly_percentage,
            'purchase_fee_months' => $this->purchase_fee_months,
            'purchase_fee_percentage' => $this->purchase_fee_percentage,
            'purchase_fee_flat'   => $this->stripCommas($this->purchase_fee_flat),
            'purchase_fee_flat_type' => $this->purchase_fee_flat_type,
            'purchase_fee_rental_period' => $this->purchase_fee_rental_period,
            ...
            'purchase_fee_other_commercial' => $this->purchase_fee_other_commercial,
            'purchase_fee_purchase_price' => $this->purchase_fee_purchase_price,
            'sales_tax_option_monthly' => $this->sales_tax_option_monthly,
            'nominal'             => $this->stripCommas($this->nominal),
            ...
            'landlord_broker_flate_fee_type' => $this->landlord_broker_flate_fee_type,
            'lease_type'          => $this->lease_type,
            'lease_value'         => str_replace(',', '', $this->lease_value ?? ''),
            'purchase_type'       => $this->purchase_type,
            'purchase_value'      => str_replace(',', '', $this->purchase_value ?? ''),
            'lease_option_fee_type' => $this->lease_option_fee_type,
            'lease_option_fee_flat' => $this->lease_option_fee_flat,
            ...
            'tenant_desired_lease_length' => $this->tenant_desired_lease_length,
            'security_deposit_budget' => $this->security_deposit_budget,
            'move_in_funds_available' => $this->move_in_funds_available,
            'first_month_rent_available' => $this->first_month_rent_available,
            'last_month_rent_available' => $this->last_month_rent_available,
            'pet_information'     => $this->pet_information,
            'number_of_occupants' => $this->number_of_occupants,
            'parking_needed'      => $this->parking_needed,
            'utility_preference'  => $this->utility_preference,
            'maintenance_preference' => $this->maintenance_preference,
            'renewal_option_requested' => $this->renewal_option_requested,
            'renewal_option_details' => $this->renewal_option_details,
            'tenant_conditions'   => $this->tenant_conditions,
            'additional_tenant_lease_terms' => $this->additional_tenant_lease_terms,
            'commercial_lease_type_preference' => $this->commercial_lease_type_preference,
            'cam_nnn_preference'  => $this->cam_nnn_preference,
            'rent_escalation_preference' => $this->rent_escalation_preference,
            'buildout_tenant_improvement_request' => $this->buildout_tenant_improvement_request,
            'intended_business_use' => $this->intended_business_use,
            'signage_request'     => $this->signage_request,
            'commercial_parking_access_needs' => $this->commercial_parking_access_needs,
            'personal_guarantee_preference' => $this->personal_guarantee_preference,
            'commercial_approval_conditions' => $this->commercial_approval_conditions,
    ...
    public function loadDraft($listingId)
    {
        ...
        if ($auction) {
            ...
            // well as in mount() because a loaded draft's stored user_type is the authoritative
            // one. Inert unless the tenant role's workflow is in scope; anything the reference
            ...
            // Tenant Lease Preference Questions
            $this->tenant_desired_lease_length = $auction->get->tenant_desired_lease_length ?? '';
            $this->security_deposit_budget = $auction->get->security_deposit_budget ?? '';
            $this->move_in_funds_available = $auction->get->move_in_funds_available ?? '';
            $this->first_month_rent_available = $auction->get->first_month_rent_available ?? '';
            $this->last_month_rent_available = $auction->get->last_month_rent_available ?? '';
            $this->pet_information = $auction->get->pet_information ?? '';
            $this->number_of_occupants = $auction->get->number_of_occupants ?? '';
            $this->parking_needed = $auction->get->parking_needed ?? '';
            $this->utility_preference = $auction->get->utility_preference ?? '';
            $this->maintenance_preference = $auction->get->maintenance_preference ?? '';
            $this->renewal_option_requested = $auction->get->renewal_option_requested ?? '';
            $this->renewal_option_details = $auction->get->renewal_option_details ?? '';
            $this->tenant_conditions = $auction->get->tenant_conditions ?? '';
            $this->additional_tenant_lease_terms = $auction->get->additional_tenant_lease_terms ?? '';
            $this->commercial_lease_type_preference = $auction->get->commercial_lease_type_preference ?? '';
            $this->cam_nnn_preference = $auction->get->cam_nnn_preference ?? '';
            $this->rent_escalation_preference = $auction->get->rent_escalation_preference ?? '';
            $this->buildout_tenant_improvement_request = $auction->get->buildout_tenant_improvement_request ?? '';
            $this->intended_business_use = $auction->get->intended_business_use ?? '';
            $this->signage_request = $auction->get->signage_request ?? '';
            $this->commercial_parking_access_needs = $auction->get->commercial_parking_access_needs ?? '';
            $this->personal_guarantee_preference = $auction->get->personal_guarantee_preference ?? '';
    ...
    protected function persistLocationDna($auction): void
    {
        ...
        // derives the legacy `zipCodes` mirror from canonical state rather than from a component
        // property, so a cascade ZIP selection reaches that mirror through the canonical
        // `zip_codes` dimension automatically — which is why `create_tenant` is deliberately NOT
        // a member of HasGeographyCascade::ZIP_MIRROR_WORKFLOWS. Adding it would create a second,
        // property-sourced path to the same key.
    ...
    public function store()
    {
        ...
        try {
            ...
            // the ones the user actually chose. Without this ordering the hydrate would re-read
            // the widget's server-seeded blob and validation would see the STORED geography rather
            // than the edited selection. Idempotent, and inert while the cascade is off.
            $this->applyGeographyCascadeToPayload();
