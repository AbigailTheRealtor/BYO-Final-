...
class TenantAgentAuctionEdit extends Component
{
     ...
     *
     * Must match TenantAgentAuction's map exactly. This class is the ONLY reachable Hire Buyer and
     * Hire Tenant EDIT surface — `buyer.edit-auction` routes here with user_type defaulted to
     * `buyer`, and `hire.agent.auction.edit` routes here for every role. If the two maps disagreed,
     * a listing would be created under one workflow key and edited under another, and the cascade
     * would appear on create and vanish on edit.
     *
     * `seller` and `landlord` resolve to NULL, which disables the cascade by the type of the
     * argument rather than by the contents of a config file.
     */
    ...
    public $lease_amount_frequency = '';  // Store it as an array
    ...
    protected $rules = [
        ...
        // Tenant Leasing Terms — 23 new optional fields
        'tenant_desired_lease_length'        => 'nullable|string|max:255',
        'security_deposit_budget'            => 'nullable|numeric|min:0',
        'move_in_funds_available'            => 'nullable|numeric|min:0',
        'first_month_rent_available'         => 'nullable|string|in:,Yes,No,Negotiable',
        'last_month_rent_available'          => 'nullable|string|in:,Yes,No,Negotiable',
        'pet_information'                    => 'nullable|string',
        'number_of_occupants'                => 'nullable|integer|min:0',
        'parking_needed'                     => 'nullable|string|max:500',
        'utility_preference'                 => 'nullable|string|max:500',
        'maintenance_preference'             => 'nullable|string|max:500',
        'renewal_option_requested'           => 'nullable|string|in:,Yes,No,Negotiable',
        'renewal_option_details'             => 'nullable|string',
        'tenant_conditions'                  => 'nullable|string',
        'additional_tenant_lease_terms'      => 'nullable|string',
        'commercial_lease_type_preference'   => 'nullable|string|in:,Gross Lease,Net Lease,NNN (Triple Net),Modified Gross,Absolute Net,Percentage Lease,Negotiable',
        'cam_nnn_preference'                 => 'nullable|string',
        'rent_escalation_preference'         => 'nullable|string|max:500',
        'buildout_tenant_improvement_request'=> 'nullable|string|max:500',
        'intended_business_use'              => 'nullable|string|max:500',
        'signage_request'                    => 'nullable|string|max:500',
        'commercial_parking_access_needs'    => 'nullable|string|max:500',
        'personal_guarantee_preference'      => 'nullable|string|in:,Yes,No,Negotiable',
    ...
    {
        return view('livewire.tenant-agent-auction-edit')
            ->extends('layouts.main') // Ensure this matches your layout file
            ->section('content'); // Ensure this matches your layout section
    }
    ...
    // Load auction data
    public function loadAuctionData($auctionId, $user_type)
    {
        ...
        // 9D: Search Areas + Important Places (Buyer/Tenant only). Runs after the discrete
        // cities/counties/state loads above so the blob prefill guards see them.
        if (in_array($this->user_type, ['buyer', 'tenant'])) {
            $this->loadSearchAreas($auction);

            // Phase 1c — hydrate the cascade from the SAME decoded document loadSearchAreas() just
            // produced. Booted here because the LOADED record's user_type is authoritative: the
            // route may default it, but the stored value decides which workflow this listing is.
            // Anything the corpus cannot match is preserved, never dropped.
        ...
        $this->maintenance_by = $auction->info('maintenance_by');
        $this->utilities = $auction->info('utilities');
        $this->included_storage_space_com_entire = $auction->info('included_storage_space_com_entire');
        $this->storage_space_com_entire = $auction->info('storage_space_com_entire');
        $this->common_areas_cleaning = $auction->info('common_areas_cleaning');
        $this->included_storage_space_com_single = $auction->info('included_storage_space_com_single');
        $this->storage_space_com_single = $auction->info('storage_space_com_single');
        $this->maintenance_response_time = $auction->info('maintenance_response_time');
        $this->included_storage_space_res_both = $auction->info('included_storage_space_res_both');
        ...
        $this->access_24_7 = $auction->info('access_24_7');
        $this->zoning_allows = $auction->info('zoning_allows');
        $this->shared_amenities = $auction->info('shared_amenities');
        ...
        $this->minimum_heated_square = $auction->info('minimum_heated_square');
        ...
        $this->total_square_feet = $auction->info('total_square_feet');
        $this->sqft_heated_source = $auction->info('sqft_heated_source');
        ...
        $this->other_bathrooms = $auction->info('other_bathrooms');
        $this->pets = $auction->info('pets');
        $this->number_of_pets = $auction->info('number_of_pets');
        $this->breed_of_pets = $auction->info('breed_of_pets');
        $this->type_of_pets = $auction->info('type_of_pets');
        $this->lease_date = $auction->info('lease_date');
        ...
        // Buyer Purchasing Terms — 18 new optional fields (Task #155, rehydration)
        ...
        // Landlord Lease Terms — 22 new optional fields (Task #157, rehydration)
        ...
        $this->weight_of_pets = $auction->info('weight_of_pets');
        $this->service_animal = $auction->info('service_animal');
        ...
        $this->prior_felony = $auction->info('prior_felony');
        $this->support_animal = $auction->info('support_animal');
        ...
        $this->target_closing_date = $auction->info('target_closing_date');
        $this->maximum_budget = $auction->info('maximum_budget');
        $this->other_financing = $auction->info('other_financing');
        $this->cash_budget = $auction->info('cash_budget');
        ...
        $this->prepayment_penalty = $auction->info('prepayment_penalty');
        $this->balloon_payment = $auction->info('balloon_payment');
        $this->prepayment_penalty_amount = $auction->info('prepayment_penalty_amount');
        $this->balloon_payment_amount = $auction->info('balloon_payment_amount');
        ...
        $this->additional_cash = $auction->info('additional_cash');
        $this->value_determination = $auction->info('value_determination');
        $this->lease_option_price = $auction->info('lease_option_price');
        $this->lease_option_terms = $auction->info('lease_option_terms');
        $this->lease_option_duration = $auction->info('lease_option_duration');
        $this->lease_option_payment = $auction->info('lease_option_payment');
        $this->lease_option_conditions = $auction->info('lease_option_conditions');
        $this->has_option_fee = $auction->info('has_option_fee');
        $this->option_fee_amount = $auction->info('option_fee_amount');
        $this->lease_purchase_price = $auction->info('lease_purchase_price');
        $this->lease_purchase_terms = $auction->info('lease_purchase_terms');
        $this->lease_purchase_duration = $auction->info('lease_purchase_duration');
        $this->lease_purchase_payment = $auction->info('lease_purchase_payment');
        $this->lease_purchase_conditions = $auction->info('lease_purchase_conditions');
        $this->lease_purchase_option_fee = $auction->info('lease_purchase_option_fee');
        $this->lease_purchase_option_fee_amount = $auction->info('lease_purchase_option_fee_amount');
        $this->cryptocurrency_type = $auction->info('cryptocurrency_type');
        $this->cash_percentage_crypto = $auction->info('cash_percentage_crypto');
        $this->crypto_percentage = $auction->info('crypto_percentage');
        $this->nft_percentage = $auction->info('nft_percentage');
        ...
        $this->interested_in_selling_type = $auction->info('interested_in_selling_type');
        $this->landlord_broker_purchase_price = $auction->info('landlord_broker_purchase_price');
        ...
        $this->broker_fee_timing = $auction->info('broker_fee_timing');
        $this->broker_fee_days_from_rent = $auction->info('broker_fee_days_from_rent');
        $this->broker_fee_days_after_lease = $auction->info('broker_fee_days_after_lease');
        $this->broker_fee_days_after_rent = $auction->info('broker_fee_days_after_rent');
        $this->broker_fee_timing_other = $auction->info('broker_fee_timing_other');
        $this->split_payment_due = $auction->info('split_payment_due');
        $this->split_payment_due_other = $auction->info('split_payment_due_other');
        $this->broker_fee_days_after_due_event = $auction->info('broker_fee_days_after_due_event');




        $this->renewal_fee_type = $auction->info('renewal_fee_type');
        $this->renewal_fee_percentage = $auction->info('renewal_fee_percentage');
        $this->renewal_fee_lease_value = $auction->info('renewal_fee_lease_value');
        $this->renewal_fee_first_month = $auction->info('renewal_fee_first_month');
        $this->renewal_fee_flat_free = $auction->info('renewal_fee_flat_free');
        $this->renewal_fee_custom = $auction->info('renewal_fee_custom');



        $this->renewal_fee_sales_tax_lease_value = $auction->info('renewal_fee_sales_tax_lease_value');
        $this->renewal_fee_sales_tax_flat_fee = $auction->info('renewal_fee_sales_tax_flat_fee');
        $this->renewal_fee_sales_tax_first_month = $auction->info('renewal_fee_sales_tax_first_month');
        $this->renewal_fee_no_of_months = $auction->info('renewal_fee_no_of_months');
        $this->expansion_commission_percentage = $auction->info('expansion_commission_percentage');
        $this->interested_in_property_management = $auction->info('interested_in_property_management');
        $this->interested_in_property_management_fee = $auction->info('interested_in_property_management_fee');
        $this->interested_in_property_management_fee_gross_lease = $auction->info('interested_in_property_management_fee_gross_lease');
        $this->interested_in_property_management_fee_rental_periord = $auction->info('interested_in_property_management_fee_rental_periord');
        $this->interested_in_property_management_fee_flate_free = $auction->info('interested_in_property_management_fee_flate_free');
        $this->interested_in_property_management_fee_other = $auction->info('interested_in_property_management_fee_other');


        $this->tenant_broker_commission_structure = $auction->info('tenant_broker_commission_structure');
        $this->tenant_broker_fee_structure = $auction->info('tenant_broker_fee_structure');
        $this->tenant_broker_gross_lease = $auction->info('tenant_broker_gross_lease');
        $this->tenant_broker_percentage = $auction->info('tenant_broker_percentage');
        $this->tenant_broker_first_month_rent = $auction->info('tenant_broker_first_month_rent');
        $this->tenant_broker_flat_fee = $auction->info('tenant_broker_flat_fee');
        $this->tenant_broker_other = $auction->info('tenant_broker_other');
        ...
        $this->commission_structure_type_fee_flat = $auction->info('commission_structure_type_fee_flat');
        $this->commission_structure_type_fee_percentage = $auction->info('commission_structure_type_fee_percentage');
        $this->commission_structure_type_fee_percentage_combo = $auction->info('commission_structure_type_fee_percentage_combo');
        $this->commission_structure_type_fee_flat_combo = $auction->info('commission_structure_type_fee_flat_combo');
        $this->commission_structure_type_fee_other = $auction->info('commission_structure_type_fee_other');
        $this->seller_leasing_fee_type = $auction->info('seller_leasing_fee_type');
        $this->seller_leasing_gross = $auction->info('seller_leasing_gross');
        $this->sales_tax_option_gross = $auction->info('sales_tax_option_gross');
        $this->seller_leasing_gross_rental = $auction->info('seller_leasing_gross_rental');
        $this->seller_leasing_gross_month_rent = $auction->info('seller_leasing_gross_month_rent');
        $this->seller_leasing_gross_sales_tax_first_month = $auction->info('seller_leasing_gross_sales_tax_first_month');
        $this->seller_leasing_gross_no_of_months = $auction->info('seller_leasing_gross_no_of_months');
        $this->seller_leasing_each_rental = $auction->info('seller_leasing_each_rental');
        $this->seller_leasing_gross_flat_combo = $auction->info('seller_leasing_gross_flat_combo');
        $this->seller_leasing_gross_percentage_combo = $auction->info('seller_leasing_gross_percentage_combo');
        $this->seller_leasing_gross_flat_net_combo = $auction->info('seller_leasing_gross_flat_net_combo');
        $this->seller_leasing_gross_sales_tax_option_gross = $auction->info('seller_leasing_gross_sales_tax_option_gross');
        $this->seller_leasing_gross_ross_percentage_rent = $auction->info('seller_leasing_gross_ross_percentage_rent');
        $this->seller_leasing_gross_percentage_no_of_months = $auction->info('seller_leasing_gross_percentage_no_of_months');
        $this->seller_leasing_gross_sales_tax_flat_free_gross = $auction->info('seller_leasing_gross_sales_tax_flat_free_gross');
        ...
        $this->purchase_fee_percentage_combo = $auction->info('purchase_fee_percentage_combo');
        $this->purchase_fee_flat_combo = $auction->info('purchase_fee_flat_combo');
        $this->purchase_fee_other = $auction->info('purchase_fee_other');
        $this->purchase_fee_other_commercial = $auction->info('purchase_fee_other_commercial');
        $this->purchase_fee_purchase_price = $auction->info('purchase_fee_purchase_price');
        $this->purchase_fee_flat_commercial = $auction->info('purchase_fee_flat_commercial');
        $this->purchase_fee_net_aggregate = $auction->info('purchase_fee_net_aggregate');
        $this->purchase_fee_monthly_percentage = $auction->info('purchase_fee_monthly_percentage');
        $this->purchase_fee_months = $auction->info('purchase_fee_months');
        $this->purchase_fee_gross_rent = $auction->info('purchase_fee_gross_rent');
        $this->sales_tax_option_flat = $auction->info('sales_tax_option_flat');
        $this->sales_tax_option_monthly = $auction->info('sales_tax_option_monthly');
        $this->lease_option_fee_type = $auction->info('lease_option_fee_type');
        $this->lease_option_fee_flat = $auction->info('lease_option_fee_flat');
        $this->lease_fee_flat_combo_net = $auction->info('lease_fee_flat_combo_net');
        $this->lease_option_fee_percentage = $auction->info('lease_option_fee_percentage');
        $this->lease_fee_percentage_net = $auction->info('lease_fee_percentage_net');
        $this->lease_option_fee_other = $auction->info('lease_option_fee_other');
        $this->protection_period = $auction->info('protection_period');
        $this->early_termination_fee_option = $auction->info('early_termination_fee_option');
        $this->early_termination_fee_amount = $auction->info('early_termination_fee_amount');
        $this->retainer_fee_option = $auction->info('retainer_fee_option');
        $this->retainer_fee_amount = $auction->info('retainer_fee_amount');
        $this->retainer_fee_application = $auction->info('retainer_fee_application');
        $this->agency_agreement_timeframe = $auction->info('agency_agreement_timeframe');
        $this->agency_agreement_custom = $auction->info('agency_agreement_custom');
        $this->brokerage_relationship = $auction->info('brokerage_relationship');
        $this->person_meeting = $auction->info('person_meeting');
        $this->meeting_details_first_name = $auction->info('meeting_details_first_name');
        $this->meeting_details_last_name = $auction->info('meeting_details_last_name');
        ...
        $this->meeting_details_instructions = $auction->info('meeting_details_instructions');
        $this->service_completion_date = $auction->info('service_completion_date');
        $this->service_completion_time = $auction->info('service_completion_time');
        $this->service_time_zone = $auction->info('service_time_zone');
        $this->meeting_details_additional_details = $auction->info('meeting_details_additional_details');
        $this->list_criteria = $auction->info('list_criteria');
        $this->list_criteria_fee = $auction->info('list_criteria_fee');
        $this->market_groups = $auction->info('market_groups');
        $this->market_groups_fee = $auction->info('market_groups_fee');
        $this->promote_social = $auction->info('promote_social');
        $this->promote_social_fee = $auction->info('promote_social_fee');
        $this->launch_ads = $auction->info('launch_ads');
        $this->launch_ads_fee = $auction->info('launch_ads_fee');
        $this->include_marketing_fee = $auction->info('include_marketing_fee');
        $this->marketing_materials_fee = $auction->info('marketing_materials_fee');
        $this->email_notifications_fee = $auction->info('email_notifications_fee');
        $this->off_market_search_fee = $auction->info('off_market_search_fee');
        $this->mls_filter_fee = $auction->info('mls_filter_fee');
        $this->email_marketing_fee = $auction->info('email_marketing_fee');
        $this->schedule_showings = $auction->info('schedule_showings');
        $this->number_of_showings_to_schedule = $auction->info('number_of_showings_to_schedule');
        $this->schedule_showings_fee = $auction->info('schedule_showings_fee');
        $this->attend_showings = $auction->info('attend_showings');
        $this->number_of_showings_to_attend = $auction->info('number_of_showings_to_attend');
        $this->attend_showings_fee = $auction->info('attend_showings_fee');
        $this->provide_virtual_tours = $auction->info('provide_virtual_tours');
        $this->number_of_virtual_tours = $auction->info('number_of_virtual_tours');
        $this->virtual_tours_fee = $auction->info('virtual_tours_fee');
        $this->assist_application = $auction->info('assist_application');
        $this->assist_application_fee = $auction->info('assist_application_fee');
        $this->collect_documents = $auction->info('collect_documents');
        $this->collect_documents_fee = $auction->info('collect_documents_fee');
        $this->submit_application = $auction->info('submit_application');
        $this->submit_application_fee = $auction->info('submit_application_fee');
        $this->review_lease = $auction->info('review_lease');
        $this->review_lease_fee = $auction->info('review_lease_fee');
        $this->provide_lease_form = $auction->info('provide_lease_form');
        $this->provide_lease_form_fee = $auction->info('provide_lease_form_fee');
        $this->coordinate_signing = $auction->info('coordinate_signing');
        $this->coordinate_signing_fee = $auction->info('coordinate_signing_fee');
        $this->prepare_application_fee = $auction->info('prepare_application_fee');
        $this->move_in_inspection_fee = $auction->info('move_in_inspection_fee');
        $this->purchase_fee_flat_type = $auction->info('purchase_fee_flat_type');
        $this->moving_resources_fee = $auction->info('moving_resources_fee');
        $this->short_term_housing_fee = $auction->info('short_term_housing_fee');
        $this->rental_rights_fee = $auction->info('rental_rights_fee');
        $this->lease_advice_fee = $auction->info('lease_advice_fee');
        ...
        $this->understand_terms = $auction->info('understand_terms');
        ...
        $this->agent_brokerage = $auction->info('agent_brokerage') ?? '';
        $this->agent_license_number = $auction->info('agent_license_number') ?? '';
        $this->agent_nar_member_id = $auction->info('agent_nar_member_id') ?? '';
    ...
    protected function persistLocationDna($auction): void
    {
        ...
        // same default mirror set, same single argument.
    ...
    {
        // 9D: block save-edit when a started Important Place row is incomplete (Buyer/Tenant,
        ...
        try {
            ...
            if (!$this->_isDraftSave) {
                $_svErrors = [];
                if (empty(trim($this->listing_title ?? ''))) $_svErrors[] = 'Listing Title';
            ...
            // $auction = HireTenantAgentAuction::findOrFail($this->auctionId);
            ...
            $auction->saveMeta('working_with_agent', $_lockedWwa ?: $this->working_with_agent);
            ...
            if ($this->video instanceof TemporaryUploadedFile) {
                ...
                // Update the Livewire property to immediately reflect the change
                $this->video = $videoName; // Set to the saved file name
            }
