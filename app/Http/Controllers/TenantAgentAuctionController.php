...
class TenantAgentAuctionController extends Controller
{
    ...
    public function list(Request $request)
    {
        ...
        $pendingApprovalAuctions = TenantAgentAuction::where(['user_id' => Auth::user()->id, 'is_approved' => false, 'is_sold' => false, 'is_draft' => false])->with(['bids.user', 'bids.meta']);
        $liveAuctions = TenantAgentAuction::where(['user_id' => Auth::user()->id, 'is_approved' => true, 'is_sold' => false, 'is_draft' => false])->with(['bids.user', 'bids.meta']);
        $soldAuctions = TenantAgentAuction::where(['user_id' => Auth::user()->id, 'is_approved' => true, 'is_sold' => true, 'is_draft' => false])->with(['bids.user', 'bids.meta']);
        if ($type == "1") {
    ...
    public function view($id, Request $request)
    {
        ...
        // Hire also mirrors a legacy `zipCodes` list; it joins the area rows.
        $page_data['hireLocationDna'] = app(\App\Services\LocationDna\ListingLocationDnaViewData::class)
            ->forSearch($auction, ['zipCodes']);
    ...
    public function admin_list(Request $request)
    {
        ...
        if ($type == 1) {
            $page_data['auctions'] = TenantAgentAuction::where('is_approved', true)->where('is_draft', false)->get();
        } else if ($type == 2) {
            $page_data['auctions'] = TenantAgentAuction::where('is_sold', true)->where('is_draft', false)->get();
        } else {
            $page_data['auctions'] = TenantAgentAuction::where('is_approved', false)->where('is_draft', false)->get();
        }
