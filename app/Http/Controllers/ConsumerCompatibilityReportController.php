...
class ConsumerCompatibilityReportController extends Controller
{
    ...
    /**
     * Show the consumer-safe compatibility insight view.
     *
     * Only permitted fields are passed to the view:
     *   - summary_sentence
     *   - per-dimension: label (dimension name), alignment_category, sentence
     *
     * Internal keys (explanation_key, template_id, trace data, reviewer notes,
     * review history, admin metadata, overall_score, deal_breaker_flags) are
     * never passed to the view.
     */
    public function show(int $id)
    {
        ...
        return response()
            ->view('consumer.compatibility_report', compact('summarySentence', 'dimensions'))
