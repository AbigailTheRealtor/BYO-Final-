...
> ### ⏱ Superseded by Phase 1 — 2026-07-26
>
> **Everything above in §3 is preserved as written on 2026-07-25 and has not been edited.** It is accurate as a record of what the codebase looked like when the audit was taken, and it is no longer an accurate description of the codebase today. This note records what changed; it does not correct the findings in place.
>
> **What Phase 1 changed.** The Scope A table's two `❌ forked copy` rows are resolved. All four Create-listing flows now delegate to the shared `<x-byo-address-autocomplete>`, and `fillFromGooglePlaces()` — defined **five times** at audit date — is now a single `fillFromResolvedAddress()` in `HandlesGooglePlacesAddress`. 240 lines of duplicated Autocomplete JS were removed. The component docblock quoted above described the follow-up cleanup as "documented; not a launch blocker"; that cleanup is the work Phase 1 completed.
>
> **Where the implemented structure differed from what §3 recorded.** The table attributes the fork to "inline `<input>` + ~60 lines of duplicated Autocomplete JS" in the four listing blades. The inline `<input>`s are **not** in those blades — the street and unit inputs live in two property-preference partials that are each included by six listing blades across all four roles. The JS was in the blades; the markup never was. This mattered: it is why adoption was implemented script-only behind a `render-markup` prop, and why the invocation had to be gated away from the Buyer and Tenant pages that render the same partials without implementing a fill method.
>
> **What §3 got right and still stands.** The Scope B/C finding is unchanged — `map-input.blade.php` remains one shared file across eight pages and the highest-leverage file in the project. The two Hire flows were correctly recorded as already sharing the component. The **12 server-side Google Autocomplete proxies are untouched** by Phase 1 and remain exactly as described; they leave with Decision D1. Roughly 40 Blade files outside the offer-listing tree still build their own Autocomplete.
>
> For the completed architecture, the consolidation summary and the commit references, see **Phase 1** in [`spatial-integration-roadmap.md`](./spatial-integration-roadmap.md).
