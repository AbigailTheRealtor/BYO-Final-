...
**Deferred (Medium/Low):** BYA MED-1…24 + LOW-1…15, BYO M1…M20 + L1…L10 carried forward at their severity. Most are post-launch; a few are launch-config/UX-honesty items to confirm against the V1 checklist (e.g. **BYO-L2** `SAVE_AS_NEW_DRAFT` still `true` in all 4 create components — must flip after C1 IDOR tests are green; **BYO-M4** placeholder "coming soon" cards render unconditionally; **BYO-M7** vacant-land mislabeled). These will be classified Required-vs-Post-launch when the itemized checklist is mapped.
