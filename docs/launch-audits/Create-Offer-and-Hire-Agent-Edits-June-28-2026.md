...
| **A3.24** | Match Create Buyer/Tenant location format for **Hire Buyer's** + **Hire Tenant's** Agent. | High | Buyer, Tenant | Hire | ✅ (**9D** — Hire Buyer/Tenant now reuse the shared `partials/location-dna/map-input` Search Areas map + Important Places + the `location_dna_preferences`/`important_places_json` meta, via new shared `HasSearchAreas` trait + `HasImportantPlaces`. Wired into BOTH Hire component trees: live catch-all `TenantAgentAuction(/Edit)` and dedicated `HireBuyerAgent\BuyerAgentAuction(/Edit)` at `/add-auction`. Duplicate City/County/State controls removed — map is the single editing surface; legacy state/counties/cities prefill the blob (non-empty guards) and mirror back to discrete meta for Ask AI/matching/display. New `HireSearchAreasParityTest` 9/9 green) |
...
| ID | Item | Priority | Roles | Flows | Status |
|----|------|----------|-------|-------|--------|
| **B1.1** | Rename/reorganize "Location Preferences Map" → **Search Areas** with helper text ("Choose where the property search should focus…"). | High | Buyer, Tenant | Create Offer | ✅ (9A — header + helper text updated in shared `partials/location-dna/map-input.blade.php`; ripples to all 6 includers) |
| **B1.2** | Order: 1 Preferred Cities, 2 Preferred ZIP Codes, 3 Preferred Counties, 4 Preferred State, 5 Draw Custom Areas on Map, 6 Interactive Map, 7 Radius Search, 8 Important Places/Commute, 9 Flexible Location, 10 Location Notes. | High | Buyer, Tenant | Create Offer | 🟡 (9A — existing controls already in target relative order (Cities→ZIP→Counties→Draw→Map→Radius→Flexible→Notes); Preferred State slot (pos 4) added in 9B, Important Places slot (pos 8) in 9C) |
| **B1.3** | Autocomplete (type-ahead) for Preferred Cities, ZIP Codes, Counties, State. | High | Buyer, Tenant | Create Offer | ⬜ |
| **B1.4** | Remove duplicate Acceptable State / Acceptable Counties UI above the map (Buyer/Tenant only). Do not delete data/columns. | High | Buyer, Tenant | Create Offer | ⬜ **Re-scoped to 9B** (coupled with B1.5 write-back — removing the duplicate UI before the discrete `state`/`counties` meta write-back exists would starve Ask AI + public views; must land together) |
...
| **B1.6** | Radius Search labels: "Radius Search Address" (placeholder "Enter an address or place for radius search"), "Radius Miles", button "Add Radius Search". | Medium | Buyer, Tenant | Create Offer | ✅ (9A — "Radius Search" section label + "Radius Search Address"/"Radius Miles" field labels + updated placeholder; button already "Add Radius Search"; ids/handlers unchanged) |
