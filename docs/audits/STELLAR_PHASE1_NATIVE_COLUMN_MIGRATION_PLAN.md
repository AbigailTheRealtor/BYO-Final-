...
> **Phase 0 Validation Gate:** PASSED — see `docs/audits/STELLAR_PHASE0_DATA_VALIDATION_REPORT.md`  
> **Phase 0 Verdict:** GO WITH ADJUSTMENTS — 17 fields confirmed, 2 Caution, 1 Blocked  
> **Adjusted Phase 1 column count:** 19 (furnished removed; see Section 9)
...
9. [Phase 2 Demotion List](#9-phase-2-demotion-list)
...
Promote the **top 19 Stellar MLS fields** (adjusted from 20 per Phase 0 validation — `furnished` demoted; see Section 9) from `raw_json` into native PostgreSQL columns on the `bridge_properties` table. This single migration will:
...
This document provides the implementing developer with everything required to execute Phase 1: the exact DDL types for all 19 promoted columns (adjusted from 20 — `furnished` removed per Phase 0 Block verdict; see Section 9), the indexing strategy including partial indexes for low-cardinality boolean columns, the backfill Artisan command specification, the exact import mapping changes for `ImportBridgeProperties.php`, the rollout phase sequence with rationale, a risk analysis with eight risk areas and their mitigations, and a post-implementation verification checklist. No code or schema changes are made as part of producing this document.

---

## 2. Top 20 Columns to Add

All 19 promoted columns are **nullable** (`furnished` excluded per Phase 0 Block verdict — see Section 9). Nullable columns allow the schema migration to run at zero downtime (the import command continues writing to `raw_json` while the new columns exist but are empty), and they allow partial backfill without constraint violations on rows where the source JSON field is absent or null.
...
| 8 | `association_fee` | `AssociationFee` | `DECIMAL(10,2)` | YES | NO | High for buyer HOA cost range queries; 22/25 populated; Ask AI context. **⚠ CAUTION (Phase 0): 72.2% populated at scale — matching engine must handle NULLs with neutral score (see Section 7h and 7a).** | HOA fee range filter plus Ask AI ownership cost answers |
...
| 10 | ~~`furnished`~~ | ~~`Furnished`~~ | ~~`VARCHAR(50)`~~ | — | — | **✗ BLOCKED (Phase 0): 35% populated in for-sale feed — removed from Phase 1 DDL. Moved to Phase 2. See Section 9.** | Deferred to Phase 2 (rental feed gate) |
...
| 20 | `cdd_yn` | `STELLAR_CDDYN` | `BOOLEAN` | YES | YES — partial (`WHERE cdd_yn = TRUE`) | Medium for buyer matching — FL CDD cost gate; 25/25 populated; Ask AI context. **⚠ CAUTION (Phase 0): 76.2% populated at scale — matching engine must handle NULLs with neutral score (see Section 7h and 7a).** | CDD filter for buyer matching plus Ask AI cost answers |
...
| ~~`furnished`~~ | ~~`Furnished`~~ | **Excluded from Phase 1** — Phase 0 Block (35% population). Deferred to Phase 2R. |
...
`BridgeApiService::fetchProperties()` already returns the full record array from the Stellar OData API, including all 20 source fields. No changes to `BridgeApiService` are needed. Only the `updateOrCreate` mapping array inside `ImportBridgeProperties::handle()` needs to be extended with the 19 promoted columns (`furnished` is excluded).
...
When Phase B of the rollout is executed, add the 19 promoted columns to `BridgeProperty::$fillable` and the appropriate casts to `BridgeProperty::$casts` (`furnished` excluded per Phase 0):

**`$fillable` additions (19 columns — `furnished` excluded per Phase 0):**
```php
...
'pets_allowed', 'garage_yn', 'pool_private_yn', 'waterfront_yn',
...
String columns (`county_or_parish`, `property_sub_type`, `mls_status`, `pets_allowed`) do not require explicit casts — Laravel returns them as strings by default. (`furnished` is excluded from Phase 1 — no cast entry needed.)
...
Schema::table('bridge_properties', function (Blueprint $table) {
    ...
    // furnished intentionally omitted — Phase 0 Block (35% population); deferred to Phase 2R
...
**What:** Add the 19 promoted column names to `$fillable` and the appropriate type casts to `$casts` in `app/Models/BridgeProperty.php` (`furnished` excluded per Phase 0; see Section 5 for the exact additions).
...
**What:** With all 19 promoted native columns populated and indexed, the buyer and tenant matching engines can be built to query native columns directly. The `raw_json` column remains the source of truth for fields not yet promoted — matching queries that need Tier 2 or display fields can extract them from `raw_json` on the already-filtered result set (which is now small enough that per-row JSON extraction is acceptable).
...
**Rollback path:** Because all 19 promoted columns are nullable and the existing `raw_json` column remains the source of truth throughout this entire process, rolling back is clean and non-destructive:
...
**`Schema::dropColumn` list for the `down()` method (19 columns — `furnished` excluded per Phase 0):**
```php
$table->dropColumn([
    'latitude', 'longitude', 'county_or_parish', 'property_sub_type', 'mls_status',
    'year_built', 'association_fee', 'tax_annual_amount', 'lot_size_sqft',
...
### 7h — Phase 0 Validation Caution Fields: `association_fee` and `cdd_yn`

**Source:** `docs/audits/STELLAR_PHASE0_DATA_VALIDATION_REPORT.md` (executed 2026-06-16 against 1,000 live Stellar records).

**Measured rates:**
- `association_fee` (`AssociationFee`): **72.2%** — Caution tier (50–79%). Down from 88% in the 25-record audit sample.
- `cdd_yn` (`STELLAR_CDDYN`): **76.2%** — Caution tier (50–79%). Down from 100% in the 25-record audit sample. The drop is likely explained by the fact that CDD (Community Development District) fees only apply to a subset of Florida communities; listings outside CDD boundaries may not populate this field.

**Why both fields are retained in Phase 1:**

Both fields are above the 50% Block threshold and neither is a critical field (critical fields are `latitude`, `longitude`, `county_or_parish`, `property_sub_type`, `mls_status`). Including them in Phase 1 is the correct decision because:
- The native columns remain nullable, so NULL rows impose no storage cost.
- 28% NULL rate for `association_fee` and 24% NULL rate for `cdd_yn` are acceptable if the matching engine handles NULLs gracefully (see required handling below).
- Leaving these fields in `raw_json` provides no improvement — they would still be NULL for the same records, and JSON extraction adds overhead.

**Required null-handling in the matching engine (mandatory before matching engine build begins):**

1. **`association_fee`:** When a buyer criterion specifies an HOA fee maximum (e.g. "max HOA $300/month"), listings where `association_fee IS NULL` must receive a **neutral score** for this dimension — not zero (which would penalize listings where HOA status is unknown) and not a match score (which would over-reward them). The recommended pattern is to award the buyer's neutral baseline score for this dimension and include NULL listings in results unless the buyer has explicitly specified "I require HOA presence confirmed" (in which case, join on `association_yn = TRUE` first to pre-filter). In WHERE clauses: `AND (association_fee <= ? OR association_fee IS NULL)` for inclusive matching.

2. **`cdd_yn`:** When a buyer criterion specifies "no CDD" preference, listings where `cdd_yn IS NULL` should be treated as **unknown** (not "no CDD"). The safest pattern is to include NULL listings in results and surface the NULL status in the UI ("CDD status unknown — verify with listing agent") rather than silently including or excluding them. In WHERE clauses: `AND (cdd_yn = FALSE OR cdd_yn IS NULL)` for inclusive matching when buyer prefers no CDD.

**Impact on the rollout DDL:** No change to the DDL — both columns are included in Phase 1 as planned. The Caution notes apply only to matching engine implementation, not to the migration itself.
...
Verify all 19 promoted columns appear in the table schema:
...
Expected: The result set includes all 19 promoted column names with the correct data types (`numeric` for decimals, `boolean` for flags, `character varying` for varchar, `smallint` for year_built, `integer` for lot_size_sqft). Note: `furnished` is **not** present — it was excluded from Phase 1 per Section 9.
...
For each of the 19 promoted columns, verify null counts match expected values after backfill. For 25-record sample tables, all reliably-populated source fields (25/25 in the audit) should show zero NULLs:
...
SELECT
    ...
    -- furnished excluded from Phase 1 DDL; column does not exist
    COUNT(*) TOTAL
FROM bridge_properties;
```

Expected for 25/25 source fields: null count = 0. Expected for 22/25 (`association_fee`): null count ≤ 3. Expected for `pets_allowed` (24/25): null count ≤ 1. Note: `furnished` is not queried here — it was excluded from Phase 1 DDL (see Section 9).
...
## 9. Phase 2 Demotion List

> **Source:** `docs/audits/STELLAR_PHASE0_DATA_VALIDATION_REPORT.md` — executed 2026-06-16 against 1,000 live Stellar records.

The following field was removed from the Phase 1 migration DDL based on Phase 0 validation results. It is scheduled for Phase 2 promotion once the data conditions described below are met.

### `furnished` (`Furnished`) — Blocked at Phase 0

| Item | Value |
|---|---|
| Stellar JSON Key | `Furnished` |
| Planned SQL Type | `VARCHAR(50)` |
| Audit-sample baseline | 9/25 (36%) |
| Phase 0 measured rate | 35% (350 / 1,000 records) |
| Threshold tier | Block (<50%) |
| Phase 1 status | **Removed from DDL** |

**Why it was blocked:** A 35% population rate in the residential-for-sale feed means 65% of all Phase 1 records will have a NULL `furnished` column. Promoting it to a native column at this rate creates a schema dimension where the majority of rows are NULL — misleading schema readers and producing unreliable WHERE clause behavior for any matching engine query that filters on it.

**Why the low rate is expected:** `Furnished` in a residential-for-sale listing describes whether the home is being sold with furniture included. This is a minority arrangement in the Florida for-sale market. The field was always expected to be sparse in the for-sale feed; the 25-record audit-sample baseline of 9/25 (36%) was the warning signal that this validation confirmed at scale.

**Phase 2 promotion gate:** Promote `furnished` to a native column when either of the following conditions is met:

1. **Rental feed activation:** The Stellar For Lease feed (`STELLAR_ForLeaseYN = true` or `LeaseConsideredYN = true`) is enabled and its records are being imported into `bridge_properties`. In the rental feed, `Furnished` is a primary tenant-facing filter (furnished vs. unfurnished apartment) and is expected to populate at a much higher rate (target: ≥80% of rental records). Re-run `php artisan bridge:validate-phase0` scoped to rental records after the rental feed is active to confirm the rate before promoting.

2. **Feed configuration change:** Stellar MLS or the Bridge Data Output API configuration is updated in a way that increases `Furnished` population in the for-sale feed above the 50% Caution threshold. Re-run the validation command to confirm.

**Impact on Phase 1 DDL:** The `furnished` column must be **removed** from the `$table->string('furnished')->nullable()` line in the Phase 1 migration file before it is run. The `$fillable` and `$casts` additions in `BridgeProperty.php` must also omit `furnished`. The `updateOrCreate` mapping in `ImportBridgeProperties` must also omit the `furnished` field mapping until Phase 2 promotion is approved.

**Impact on the matching engine:** The tenant matching engine's furnished/unfurnished toggle cannot rely on a native `furnished` column until Phase 2. For the initial matching engine build, extract `furnished` from `raw_json` at query time (acceptable for per-record context loading via Ask AI or listing display; not acceptable for a bulk WHERE-clause filter across the full table). Document this limitation as a known MVP gap in the matching engine.
