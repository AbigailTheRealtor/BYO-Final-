...
- [x] Golden-master ranking parity: **995/995 scored rows** byte-identical (`golden_hash_sha256 = cfc0c05c…`, 103 groups, 6 listings). — the "1,090/1,090" wording conflates scored + unscored rows; the frozen corpus is the 995 rows that carry a `ranking_score` (the other 95 of 1,090 are unscored and excluded). See **E-10** / **E-48**. *(Digest re-baselined to `498b683c…` by **Batch 2D Part B** after the golden master was fully de-Googled — names, ratings, reviews, real coordinates, and listing ids all replaced with deterministic synthetic content; `expected` + digest regenerated. Structure and determinism unchanged; see `docs/spatial/B2D-part-b-hybrid-gate-1-harness.md`.)*
...
**This conflates two different populations.** `property_location_pois` holds **1,090 rows across 13 listings**, but only **995 rows across 6 listings** carry a `ranking_score` — the frozen golden master (`golden_hash_sha256 = cfc0c05c…`, re-baselined to `498b683c…` by Batch 2D Part B after full de-Googling — names, ratings, reviews, coordinates, and listing ids all synthesised, 103 listing×category groups). The other **95** rows are unscored (no `ranking_score`) and are correctly excluded from the parity corpus; separately, only **844** of the rated rows carried a rating (the original E-10 observation) — all now removed from the fixture by Batch 2D Part B.
