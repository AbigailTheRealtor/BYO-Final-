...
**Untouched by this programme:** `initializeLimitedService()`, the `TenantAgentAuction` Livewire component, and `LOCKED_BidComparison` scoring. See INV-8. **Per SIA-D34 the freeze also covers `TenantAgentAuctionEdit`. Between them they hold five direct Google calls this freeze prevents us from instrumenting — a known exception to INV-11, recorded in E-38 and blocking on Q12.**
...
**Done when:** every Google surface degrades honestly with the key unset · telemetry emitting with HTTP status · worker consumes the queue · **no regressions introduced relative to the established pre-Phase-0 baseline, with `GOOGLE_PLACES_API_KEY` unset and zero outbound attempts** (amended by E-47 — the original *"suite green"* was unmeetable) · `SELECT postgis_version()` succeeds · INV-1, INV-8, INV-11, INV-12 hold.
...
| **INV-11** | **No test depends on a Google credential.** The suite passes with `GOOGLE_PLACES_API_KEY` unset. ⚠️ **Enforced in tests via `tests/bootstrap.php` (E-37). Production telemetry coverage is incomplete until Q12 closes — see E-38, E-40, SIA-D35.** | Guard test; CI runs with the key absent |
...
| **SIA-D34** | **INV-8's freeze extends to `TenantAgentAuctionEdit.php`.** Resolving Q11, the product owner froze both `TenantAgentAuction.php` (3 Google clients) and `TenantAgentAuctionEdit.php` (2), pending an explicit, narrowly-scoped exception. **Five Google call sites are therefore frozen and uninstrumentable.** No batch may modify them without a fresh product-owner approval naming the files. | **Approved — Binding** |
...
| **E-38** | **The bare-Guzzle census in E-35 and in the Batch 1 report was undercounted.** Both audits grepped for `new Client(` and therefore missed every **fully-qualified** `new \GuzzleHttp\Client()`. The true count of bare clients issuing requests to `maps.googleapis.com` outside `app/Services/LocationDna/` was **18, across 12 files** — not 4. Every one bypasses `GoogleOutboundTelemetryMiddleware`, so **INV-11's "zero outbound attempts" cannot be asserted from telemetry until they are routed through the container.** Three were fixed in `TenantOfferListing.php` (`f3dab6f24`) and ten more across nine files in Batch 3 (`841147d9f`), leaving **5 — all frozen**. **Three of those are in `TenantAgentAuction.php` (lines 2214, 2330, 2440), which is frozen under INV-8 and was not touched.** Two more are in `TenantAgentAuctionEdit.php`, frozen by SIA-D34 — **five frozen call sites in total, and the only ones that remain.** The role-symmetric spread (Seller / Buyer / Landlord / Tenant × Auction / OfferListing × Create / Edit) is exactly what `CLAUDE.md`'s quadruplication rule predicts. The correct detection pattern is `grep -rnE 'new\s+\\?(GuzzleHttp\\)?Client\s*\('`. **This census covers Guzzle only — see E-40 for the `Http`-facade caller it missed.** See **Q12**. | `grep -rnE` census, 2026-07-09 |

**E-38 — full census.** Frozen rows are blocked on Q12; the rest await batch scoping.

| File | Google clients | Status |
|---|---|---|
| `app/Http/Livewire/TenantAgentAuction.php` | 3 | 🔒 **Frozen (INV-8) — known exception, see Q12** |
| `app/Http/Livewire/OfferListing/Tenant/TenantOfferListing.php` | 3 | ✅ Fixed in `f3dab6f24` |
| `app/Http/Livewire/TenantAgentAuctionEdit.php` | 2 | 🔒 **Frozen by SIA-D34** — extends INV-8 pending an explicit narrow exception |
| `app/Http/Livewire/OfferListing/Tenant/TenantOfferListingEdit.php` | 2 | ✅ Fixed in `841147d9f` |
| `app/Http/Livewire/HireBuyerAgent/BuyerAgentAuction.php` | 1 | ✅ Fixed in `841147d9f` |
| `app/Http/Livewire/HireBuyerAgent/BuyerAgentAuctionEdit.php` | 1 | ✅ Fixed in `841147d9f` |
| `app/Http/Livewire/HireSellerAgent/SellerAgentAuctionEdit.php` | 1 | ✅ Fixed in `841147d9f` |
| `app/Http/Livewire/HireLandLordAgent/LandLordAgentAuctionEdit.php` | 1 | ✅ Fixed in `841147d9f` |
| `app/Http/Livewire/OfferListing/Buyer/BuyerOfferListing.php` | 1 | ✅ Fixed in `841147d9f` |
| `app/Http/Livewire/OfferListing/Buyer/BuyerOfferListingEdit.php` | 1 | ✅ Fixed in `841147d9f` |
| `app/Http/Livewire/OfferListing/Seller/SellerOfferListingEdit.php` | 1 | ✅ Fixed in `841147d9f` |
| `app/Http/Livewire/OfferListing/Landlord/LandlordOfferListingEdit.php` | 1 | ✅ Fixed in `841147d9f` |
...
| **E-39** | **`Http::preventStrayRequests()` does not exist in this framework.** It was introduced in Laravel 9; this application runs **Laravel 8.83.29**. The approved S1d step, E-20, and E-37 all name it as the missing protection, so all three are corrected here. Laravel 8's `PendingRequest::buildStubHandler()` falls through to the live Guzzle handler whenever no `Http::fake()` stub matches. The behaviour is backported by `tests/Support/Http/StrayRequestGuardFactory` (bound over `Illuminate\Http\Client\Factory` in `Tests\TestCase`), which throws `StrayHttpRequestException` at that branch. **It cannot be implemented as a throwing `Http::fake(callable)`:** Laravel evaluates every stub callback eagerly (`->map->__invoke(…)->filter()->first()`), so a throwing callback fires even for requests another stub has already matched. | `vendor/laravel/framework` 8.83.29; `PendingRequest::buildStubHandler()` |
| **E-40** | **E-38's census counted Guzzle only, and therefore missed a Google caller.** `app/Console/Commands/GeocodeSelleryLandlordListings.php:127` calls `https://maps.googleapis.com/maps/api/geocode/json` through the **`Http` facade**, which constructs its own Guzzle client and never consults the container. It is invisible to `GoogleOutboundTelemetryMiddleware` (a container handler-stack middleware) *and* to `BlocksGooglePlacesHttpClient` (a container binding). Until Batch 2 nothing in the test suite could have stopped it reaching the live network. Guarded by E-39's factory in Batch 2, and **routed through the container in Batch 3 (`841147d9f`); zero `Http`-facade Google callers now remain.** **The Google-caller inventory is nevertheless two lists, not one: bare Guzzle clients (E-38) and `Http`-facade callers (this erratum).** Any future audit must grep both — Appendix D commands 10 and 11. | `grep -rn "Http::" app/`; `GeocodeSelleryLandlordListings.php:127` |
| **E-21 (re-baselined)** | The "43 Location DNA test files" figure is superseded by measurement, as E-21 required before any Definition-of-Done use. As of `009898269`: **26** files match `*LocationDna*`/`*Ldna*` by filename; **31** files live in `tests/Unit/Services/LocationDna/`; **81** files under `tests/` reference `LocationDna` or `Ldna` in their contents. Use the figure that matches the claim being made; none of them is 43. | `find` / `grep -rl`, 2026-07-09 |

| **E-41** | **The S1b guard test passed vacuously from Batch 1 until Batch 3.** Its comment-stripper, `#(//.*$)|(/\*.*?\*/)#ms`, applied the `/s` (dotall) flag to the **line-comment** branch, so `//.*$` greedily deleted everything from the first `//` to the end of the file. `TenantAgentAuction.php` collapsed from **257,742 bytes to 912**; every regex assertion built on the result matched an empty string and passed. **A guard test that cannot fail is worse than no guard test — it manufactures confidence.** Fixed as two passes (`#/\*.*?\*/#s` then `#//[^\n]*#`), the stripper is now itself under test, and the census assertion fails closed via `assertNotEmpty($offenders)` when the scan finds nothing at all. | `tests/Feature/Security/GooglePlacesKillSwitchTest.php`; byte-count demo |
| **E-42** | **The test suite was making live Google requests, and no guard could see them.** Fourteen tests in `CreateEditParityRegressionTest` drove Livewire update hooks that ran address autocomplete. Those methods built bare Guzzle clients, so `BlocksGooglePlacesHttpClient` — a *container* binding — never intercepted them. The tests passed because Google answers a blank key with **HTTP 200 + `REQUEST_DENIED`** and an empty `predictions` array, which the components read as "no suggestions". **Every run of that file therefore sent 14+ requests to `maps.googleapis.com`.** This is the same failure mode as the 2026-07-05 incident, differing only in that the endpoint was unbilled. It was invisible until Batch 3 routed the call sites through the container. The tests now bind `Tests\Support\AnswersGooglePlacesWithRequestDenied`, reproducing that response in-process. **Corollary: "no test made a Google call" was never verified — it was assumed, on the strength of E-41's vacuous test.** | Batch 3 fallout: 28 `BLOCKED live Google` failures, resolved to 0 |

| **E-43** | **Phase 0 item 1's address-degrade bullet is self-contradictory.** It prescribes *"Address fields → free-text entry **+ pin confirmation**"* in the same breath as *"Maps → a labelled 'Map unavailable' panel."* **Pin confirmation presupposes a map.** With no credential there is no map to drop a pin on, and under SIA-D25 there will never be a Google one. Corrected: in Phase 0 the address degrade is **free-text entry only** — the field accepts what the user types, autocomplete is silently absent, nothing is validated against Google. **Pin confirmation arrives with MapLibre in Phase 5**, and only then becomes a coherent requirement. Phase 0's Done-when clause is read accordingly. | §17 Phase 0 item 1 |
| **E-44** | **The `GOOGLE_PLACES_ENABLED` kill switch must not gate address entry.** Item 7 calls it a *"hard guard"* and item 1 calls for degradation, and the two read as though the switch should govern every Google surface. It must not. `config('google_places.enabled')` defaults to **`false`**, and it exists to stop the billable **Nearby Search enrichment** path (the $1,223 incident). If address autocomplete consulted it, wiring the switch — Batch 1 — would have silently killed address entry on all four roles' create *and* edit forms the moment it landed, which is precisely the production breakage the product owner forbade. **Autocomplete and geocoding guard on the credential's presence (`GoogleCredential`); only the enrichment path guards on the kill switch.** Pinned by `GracefulDegradationTest::the_kill_switch_does_not_gate_address_autocomplete`. | `config/google_places.php`; §17 Phase 0 items 1 & 7 |

| **E-45** | **The "7 hard-coded SDK loaders" figure was wrong; there were 5, and 4 of them were deferred.** `add_listing.blade.php`'s loader is **commented out** and the view already used the canonical component; `hire_tenant_agent/add.blade09012024.php` is a **dead, unreferenced backup**. Of the five real loaders, four (`hire_landlord_agent/edit`, `landlord_auction/add`, `landlord_auction/edit`, `seller_property/edit`) were byte-identical copies defining `loadGoogleMapsScript()` for *deferred* injection, and could not simply adopt `<x-google-maps-script>` without changing the SDK from load-on-demand to load-on-every-page-view. They were consolidated onto a new `<x-google-maps-deferred-loader>`. **Two real defects surfaced:** (a) with no credential those four defined *nothing*, so the deferred-init list threw `ReferenceError: loadGoogleMapsScript is not defined`; (b) **41 legacy `initialize()`/`initializeMap()` functions touched `google.maps` with no runtime guard.** A server-side credential check cannot cover (b) — a *present but rejected* key renders the script tag, Google refuses it, and `google` is never defined. | Batch 5 (`8d576abd2`) |
| **E-46** | **Blade compiles its directives inside JavaScript comments, and `view:cache` will not tell you.** A comment written during Batch 5 — `// the server-side @if($mapsKey) check` — compiled to a real, unterminated `if (…):` and produced HTTP 500 on four public listing views. `php artisan view:cache` reported *"Blade templates cached successfully"*, because **compiling is not executing**: the emitted PHP is never parsed. Any `@if`, `@foreach`, `@php`, or `{{ }}` in a JS or CSS comment inside a Blade file is live code. `GoogleMapsBladeGuardTest::every_google_maps_view_compiles_to_valid_php` now compiles each Google-Maps view and runs `php -l` over the output. **A suite-wide sweep found exactly one other broken view:** `resources/views/partials/_notification_type_messages.blade.php`, which is pre-existing, unreferenced by any include or route, and left untouched. | Batch 5 |

| **E-47** | **Phase 0's "suite green" exit criterion was unmeetable and is replaced.** The clause read *"suite green with `GOOGLE_PLACES_API_KEY` unset and zero outbound attempts."* The suite has **never** been green: a stashed pre-Phase-0 baseline measured **~210 Unit failures / 5,399 passing** and **120 Feature failures / 2,433 passing**, all predating this programme. Read literally, the clause blocks Phase 0 forever on unrelated historical debt. **Product-owner decision (2026-07-09):** the criterion becomes *"**no regressions introduced relative to the established pre-Phase-0 baseline**, and zero outbound attempts."* The operational test, applied unchanged since Batch 2, is: a failure counts as a regression only if it reproduces across **both** runs of the new tree **and** is absent from **both** runs of the baseline tree — the Feature suite's failing-test names churn ~18 per run, so single-run counts prove nothing. The historical failures remain a separate remediation track and do not gate this programme. | Baseline runs, Batches 1–5 |
| **E-48** | **`ranking_score` is set-relative, not a property of a POI.** `LocationDnaRankingEngine::rankCandidates()` normalises its distance component by `max($distances)` **across the candidate set it is handed**. Two consequences, both load-bearing for Phase 1. **(a)** Persisted historical scores cannot be reproduced from persisted rows wherever the set was truncated: 15 of the 995 scored rows — every one `top_rated_dining`, the only category persisted top-3 rather than top-10 — diverge, because the full fetched set was never persisted and cannot be recovered. The golden master therefore asserts *engine determinism under refactor*, freezing whole candidate sets, and does **not** claim to reproduce history. **(b)** A replacement provider returning a **different candidate set will change `ranking_score` even for byte-identical POIs.** Gate 3's baseline-diff harness must therefore compare **rank order and membership**, not raw scores, or it will report arithmetic differences as semantic ones. | `LocationDnaRankingEngine:55`; 15/995 divergence measured 2026-07-09 |
...
| ~~**Q11**~~ | ~~How do we close the frozen Google calls in `TenantAgentAuction.php`?~~ | — | **Resolved 2026-07-09 by product owner.** See SIA-D34. |
| **Q12** | **What closes the five frozen Google calls before Gate 7?** SIA-D34 froze them without choosing an endgame. Remaining options: (a) a narrow, explicitly approved INV-8 exemption limited to the client swap; (b) make the calls unreachable when `google_places.enabled=false`, from outside the frozen files; (c) accept the gap until Phase 7 deletes the Google code path wholesale. | **Gate 7 / V1 certification**; full INV-11 telemetry coverage | **Product owner** |
...
#     As of 841147d9f: 5 Google-calling bare clients remain, ALL frozen by INV-8/SIA-D34 (Q12).
#     Enforced by GooglePlacesKillSwitchTest::only_frozen_files_still_construct_a_bare_google_client.
grep -rnE 'new\s+\\?(GuzzleHttp\\)?Client\s*\(' app/ --include=*.php

# 11. Google callers via the Http facade (E-40). These bypass BOTH the container
#     binding and the telemetry middleware, so command 10 cannot see them.
#     Expected at Phase 7: zero. As of 841147d9f: zero (the sole caller was routed through the container).
grep -rn "Http::" app/ --include=*.php | grep "googleapis.com"
```
