...
The Documents & Disclosures card (Group 5) in both Seller and Landlord Tax/Legal/HOA/Disclosures tab supports conditional file uploads: when any disclosure dropdown is set to "Yes", a `wire:model` file input appears for upload. Both components use `WithFileUploads`, declare temporary upload properties (`landlord_disclosure_file`, `survey_file`, `inspection_report_file`, `hoa_condo_docs_file`, `flood_disclosure_file`, `lead_based_paint_file`, `environmental_report_file`) and corresponding `*_path` persisted properties. A unified `$disclosureUploads` loop in `saveAllMetadata()` stores files under `{role}-disclosures/{id}/{type}/` on the public disk and saves paths via `saveMeta`. `loadDraft()` restores all path values for "View current file" links. Individual `updated*File()` handlers validate mime types and 10 MB size limit. Dollar-sign inputs for `annual_cdd_fee`, `special_assessment_amount`, and `association_fee_amount` have `style="padding-left: 12px;"` for correct text alignment. The `additional_landlord_lease_terms` textarea uses `rows="3"`. The photo/video upload loader (`videoLoader`) in both listing blades now hides immediately on `upload:finish` (no 30-second setTimeout).
