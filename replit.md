...
The non-agent client dashboard (`/dashboard`) presents a single "My Listing" view. The 4-role summary grid (Tenant's Agent, Landlord's Agent, Buyer's Agent, Seller's Agent cards) has been removed. `DashboardController::dashboard()` now computes `$activeListingData` — finds the most recent listing for the user's primary role (with fallback to other roles) and passes `listing`, `role`, and `bidCount`. The blade renders one listing card with role-coloured left border, title, status badge, bid count, "View Listing" and "View Bids" buttons, or an empty-state CTA if no listing exists. Quick Actions use singular language: "Create New Listing" and "View My Listing". The sidebar for non-agents shows a single "My Listing" nav item routing dynamically to the user's role-specific listing list; agents retain the full multi-type My Listings sidebar.
