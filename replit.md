...
### Offered Services Display Fix - Private Terms Modal (December 2025)
- **Root Cause**: Private Terms modal was using keyword-based matching to categorize services, which caused misplacement into "Other Services" bucket
- **Fix Applied**: Replaced keyword matching with exact string matching using the same predefined category arrays from the Actual Listing display
- **Service Categories**: Uses identical `$residentialCategories` and `$commercialCategories` arrays with exact service strings
- **Matching Logic**: Changed from `str_contains()` keyword matching to `in_array()` exact matching
- **Property Type Detection**: Uses `property_type` from auction to select correct category set (Residential vs Commercial)
- **No False "Other Services"**: Only shows "Additional Services" when agent explicitly entered custom text in the other_services field
- **Order Preserved**: Categories display in same order as the Actual Listing (Marketing, Search, Showings, Application, Lease, Move-In, Strategy)