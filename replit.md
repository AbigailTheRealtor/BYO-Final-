...
**Display Rules**: 1 selection = plain text, 2+ selections = pill badges/chips. "Other" values are resolved to custom text via `normalizeList()`. None/N/A values are filtered out. Buyer "Acceptable Property Styles" always renders as plain text (comma-separated). "½ Duplex" is normalized to "1/2 Duplex" at display-time. Cities/counties strip state suffixes (", FL"). Financing sub-answers follow the 1=text/2+=chips rule. "Pet Types:" is the standardized label across all listing types.
