...
- **Seller edit routing**: Create flow uses Livewire (`SellerAgentAuction.php` → `hire-seller-agent.blade.php`). Edit route `/hire/agent/seller/edit/{id}` uses traditional blade (`hire_seller_agent/edit.blade.php`) + `SellerAgentAuctionController`. `SellerAgentAuctionEdit.php` Livewire component exists but is not wired to any route.
...
- **Financing field display**: Occupancy Requirement and Timing of Transfer use `text_or_other` format (plain text, not badges). Property Condition uses inline rendering (not `normalizeList`) to preserve "None" values. Yes/No + Amount fields (`has_option_fee`, `prepayment_penalty`, `balloon_payment`, `lease_purchase_option_fee`) use `yes_parenthetical` format combining value and amount into single-line display.
