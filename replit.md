...
**Buyer Agent Sync Events**: Both `BuyerAgentAuction.php` (create) and `BuyerAgentAuctionEdit.php` dispatch `buyer-agent-select2-sync` browser events on draft/edit load to hydrate Select2 fields inside `wire:ignore`. Both sync events include: `view_preference`, `non_negotiable_amenities`, `pool_type`, `offered_financing`, `services`, `lease_for`, `credit_scroe_rating`, `flat_fee_services`, `number_of_unit_type`, `condition_prop_buyer`, `property_items`. The JS handler in `property-preferences.blade.php` listens for this event and calls `syncSelect2()` for each field. `interested_lease_option` is NOT in the `$financingFieldMap['Lease Option']` clearing list — it's an independent broker-compensation field that persists regardless of financing type changes.

**Select2 + Livewire Integration**: Static-option multi-select Select2 fields use `wire:ignore` on their wrapper divs to prevent Livewire DOM-patching from destroying Select2 widget state. Dynamic multi-selects (e.g., `property_items`, `non_negotiable_amenities` whose options change by `$property_type`) do NOT use `wire:ignore` since their options must update with Livewire state. `public/js/select2-stable.js` sets a global `width: '100%'` default so Select2 renders correctly on hidden tabs without needing destroy/recreate. All `initializeFullService()` functions are guarded: each `.select2()` call checks `hasClass('select2-hidden-accessible')` before initializing, preventing flash on already-initialized fields. `initializeFullService()`/`initializeLimitedService()` are called on DOMContentLoaded, service-type switch, AND from `message.processed` hooks (restored after guarding). `select2-stable.js` provides: (a) `repairBrokenSelects()` — a safety cleanup that runs on every `message.processed` to detect selects with `select2-hidden-accessible` class but no adjacent `.select2-container` (caused by Livewire morphdom mismatches in conditional `@if` blocks) and restores them to normal visibility; (b) `initUninitialized()` — scans for uninitialized Select2 fields only (with namespaced `change.s2stable` handlers for generic Livewire model sync on `wire:model` fields); (c) a `shown.bs.tab` handler for hidden-tab rendering. `initializeFullService()` contains 30-46 nested `Livewire.hook` and `addEventListener` registrations which cause handler stacking when called repeatedly, but the Select2 guards (`hasClass('select2-hidden-accessible')`) prevent duplicate Select2 initialization. A global utility `public/js/select2-manager.js` provides `Select2Manager.init()`, `.safeSync()`, `.isOpen()`, and `.toggleOther()` for safe Select2 management. Non-Select2 hooks (phone formatting, tooltips, dropdown listeners, garage toggles) are preserved.

**JSON Bridge Pattern (Buyer Fields)**: Three buyer multi-select fields (`condition_prop_buyer`, `number_of_unit_type`, `property_items`) use a JSON bridge to bypass Select2 ↔ Livewire sync issues. Hidden `<input type="hidden" wire:model="*_json">` fields in `property-preferences.blade.php` store JSON strings. JS `setJsonModel()` writes Select2 selections as JSON to these hidden inputs. PHP `updated*Json()` hooks decode JSON into the real array properties. On save, `decodeJsonArray()` reads from the JSON bridge as source of truth. On load (draft/edit), `encodeJsonArray()` populates JSON from loaded arrays. `jsonRestoreSelect2()` runs at end of every `initializeFullService()` to sync Select2 UI from JSON state. The `property_items_json` and `condition_prop_buyer_json` inputs use `wire:model` (non-defer) so "Other" text input visibility re-evaluates immediately; `number_of_unit_type_json` uses `wire:model.defer`.
