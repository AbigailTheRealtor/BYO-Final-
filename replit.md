...
**Select2 + Livewire Integration**: Static-option multi-select Select2 fields use `wire:ignore` on their wrapper divs to prevent Livewire DOM-patching from destroying Select2 widget state. Dynamic multi-selects (e.g., `property_items`, `non_negotiable_amenities` whose options change by `$property_type`) do NOT use `wire:ignore` since their options must update with Livewire state. `public/js/select2-stable.js` sets a global `width: '100%'` default so Select2 renders correctly on hidden tabs without needing destroy/recreate. All `initializeFullService()` functions are guarded: each `.select2()` call checks `hasClass('select2-hidden-accessible')` before initializing, preventing flash on already-initialized fields. `initializeFullService()`/`initializeLimitedService()` are called ONLY on DOMContentLoaded and service-type switch — NEVER inside `message.processed` hooks. Instead, `select2-stable.js` provides a targeted `message.processed` hook that scans for uninitialized selects only (with namespaced `change.s2stable` handlers for generic Livewire model sync on `wire:model` fields) and a `shown.bs.tab` handler for hidden-tab rendering. CRITICAL: `initializeFullService()` contains 30-46 nested `Livewire.hook` and `addEventListener` registrations — calling it from `message.processed` causes exponential handler stacking. It must ONLY be called from DOMContentLoaded and service-type switch. A global utility `public/js/select2-manager.js` provides `Select2Manager.init()`, `.safeSync()`, `.isOpen()`, and `.toggleOther()` for safe Select2 management. Non-Select2 hooks (phone formatting, tooltips, dropdown listeners, garage toggles) are preserved.
