...
The non-agent client dashboard (`/dashboard`) shows a "My Listings by Role" 4-role summary grid — one card each for Tenant's Agent, Landlord's Agent, Buyer's Agent, and Seller's Agent. Each card shows listing count and links to the relevant role-specific listing list. The Quick Actions offer role-specific "My Listings" and "Bids on My Listings" shortcuts. `DashboardController::dashboard()` passes `$listingCounts` and `$pendingBidCounts` per role.
