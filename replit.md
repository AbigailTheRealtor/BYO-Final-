...
The non-agent client dashboard (`/dashboard`) shows ALL agent requests (listings) the user has across all four role types — no role priority, no user_type-based selection, no latest() fallback. `DashboardController::dashboard()` now passes `$allListings` — a flat array collected by querying all four models (TenantAgentAuction, LandlordAgentAuction, BuyerAgentAuction, SellerAgentAuction) without any priority ordering. The blade renders: (0 listings) empty state; (1 listing) "Your Active Agent Request" with one card; (2+ listings) "Your Agent Requests" with all cards stacked. Each card has role-coloured left border, role label, title, ID, status badge, bid count, "Open Request" and "View Bids" buttons. Welcome header: standalone user_type badge removed. Quick Actions: "Create New Request" and "View My Agent Request". Sidebar for non-agents: "My Agent Request" section and nav item linking to /dashboard; agents retain full multi-type My Listings sidebar unchanged.
