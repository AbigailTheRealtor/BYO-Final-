 ...
 *   5. The server remains authoritative — a publish that bypasses the browser
 *      entirely is still rejected, and ownership checks are unchanged.
 *
...
class PublishSubmitGateTest extends TestCase
{
    ...
    public function test_all_four_components_expose_the_server_required_field_contract(): void
    {
        ...
        foreach ($components as $label => $component) {
            ...
            // Asserted against fields required by main's own rules. Deliberately
            // NOT asserting `address`: that rule arrives with the Phase 0
            // address-validation work (ValidStreetAddress), which is excluded from
            // this branch. The contract under test is that the list is DERIVED from
            // whatever getConditionalRules() currently says — so it stays correct
            // both before and after that work lands.
            // The contact block is required by BOTH roles' rules. Role-specific
            // fields differ (Seller requires listing_title/property_type; Landlord
            // requires desired_lease_length), so only the shared set is asserted
            // for every component.
            foreach (['first_name', 'last_name', 'phone_number', 'email'] as $serverRequired) {
                $this->assertContains($serverRequired, $required,
                    "[{$label}] '{$serverRequired}' is required by the server's publish rules "
                    . 'and must appear in the gate list.');
            }
    ...
    /** All four views must wire in the shared authoritative gate. */
    public function test_all_four_views_include_the_shared_publish_gate(): void
    {
        foreach ([
            'seller/offer-seller-listing.blade.php'         => 'create-auction-form',
            'seller/offer-seller-listing-edit.blade.php'    => 'edit-auction-form',
            'landlord/offer-landlord-listing.blade.php'     => 'create-auction-form',
            'landlord/offer-landlord-listing-edit.blade.php' => 'edit-auction-form',
        ] as $view => $formId) {
            $src = $this->viewSource($view);
            ...
            $this->assertStringContainsString('id="' . $formId . '"', $src,
    ...
    /**
     * The Landlord create view carried TWO same-named checkFormValidity() helpers.
     *
     * The Full Service copy is the one that runs for Offer Listings, and it must
     * never disable Submit for DOM completeness. The second copy lives inside
     * initializeLimitedService(), which CLAUDE.md designates FROZEN legacy code for
     * the Limited Service flow — it is deliberately untouched, has no call site in
     * this view, and is excluded from this assertion for that reason. Narrowing the
     * scan is the point: a future edit that reintroduces a gate in Full Service must
     * fail here, without this suite ever pulling the frozen function into scope.
     */
    public function test_landlord_create_full_service_scope_has_no_legacy_button_gate(): void
    {
        $src = $this->viewSource('landlord/offer-landlord-listing.blade.php');

        $frozenAt = strpos($src, 'function initializeLimitedService()');

        $this->assertNotFalse($frozenAt,
            'initializeLimitedService() is expected in this view; if it was renamed, rescope this test.');

        $fullServiceScope = substr($src, 0, $frozenAt);

        $this->assertStringNotContainsString("classList.add('disabled')", $fullServiceScope,
            'Landlord create (Full Service) must not re-add .disabled to Submit — Bootstrap gates '
            . '.btn.disabled with pointer-events:none, which swallows the first click.');

        $this->assertStringNotContainsString("setAttribute('disabled', 'disabled')", $fullServiceScope,
            'Landlord create (Full Service) must not set the disabled attribute for DOM completeness. '
            . 'wire:loading.attr is the only permitted disabled state.');
    }

    // ─── 5 · The server stays authoritative ──────────────────────────────────

    /**
     * The gate must never be the thing that decides a publish.
     *
     * Proven with a rule this branch actually carries: the gate is client-side
     * only, so a component call that bypasses the browser entirely must still be
     * rejected by the server.
     *
     * Address-shape rejection is the other half of this contract, but it depends
     * on ValidStreetAddress, which belongs to the Phase 0 address-validation work
     * and is deliberately not on this branch. Those cases belong with that work
     * and should be added when it is integrated — not reconstructed here.
     */
    public function test_server_still_rejects_a_missing_required_field_when_the_client_gate_is_bypassed(): void
    {
        $user    = $this->agent();
        $auction = $this->makeSellerAuction($user, true);
