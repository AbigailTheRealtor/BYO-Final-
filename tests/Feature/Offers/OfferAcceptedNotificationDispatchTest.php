...
class OfferAcceptedNotificationDispatchTest extends TestCase
{
    ...
    public function test_successful_accept_dispatches_notification(): void
    {
        Notification::fake();
        ...
        $facade->method('accept')->willReturn([
            'allowed' => true,
            'reason'  => '',
        ]);
        ...
        Notification::assertSentTo($this->submittedOffer->user, OfferAcceptedNotification::class);
    }
    ...
    public function test_permission_denial_does_not_dispatch(): void
    {
        ...
        $this->mockActionsService($this->allowedActions([
            ...
            'reasons'    => array_merge($this->allowedActions()['reasons'], [
                'accept' => 'Cannot accept: not allowed.',
            ]),
        ]));

        $facade = $this->createMock(OfferWorkflowFacade::class);
        $facade->expects($this->never())->method('accept');
        $this->app->instance(OfferWorkflowFacade::class, $facade);
