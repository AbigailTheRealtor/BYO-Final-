...
/**
 * P1A — Buyer role mapper ↔ component alignment.
 *
 * Each test asserts that a key the Buyer bid form reads from $mapped:
 *   (a) exists in AgentBidMapperService::mapFromProfile() output, AND
 *   (b) is declared as a public property on BuyerAgentAuctionBid.
 *
 * All tests are pure unit tests — no database required.
 */
class BuyerMapperTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $rc = new ReflectionClass(BuyerAgentAuctionBid::class);
    ...
    private function assertKeyAligns(string $key): void
    {
        $this->assertArrayHasKey($key, $this->mapped,
            "AgentBidMapperService must emit '{$key}' for Buyer bid form");
        $this->assertContains($key, $this->componentProps,
            "BuyerAgentAuctionBid must declare public \${$key}");
    }
    ...
    /** @test */
    public function buyer_purchase_and_commission_keys_are_aligned(): void
    {
        foreach ([
    ...
    /** @test */
    public function buyer_lease_fee_keys_are_aligned(): void
    {
        foreach ([
