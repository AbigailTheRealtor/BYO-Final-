...
/**
 * OfferExpiredNotificationTest
 *
 * Verifies OfferExpiredNotification using a mocked Offer model only.
 ...
 *   (6) toDatabase()['type'] equals 'offer_expired'
 *   (7) toMail() returns an instance of MailMessage
 *   (8) toMail() subject contains the offer id
 */
class OfferExpiredNotificationTest extends TestCase
{
    private MockInterface $offer;
    private OfferExpiredNotification $notification;
    ...
    protected function setUp(): void
    {
        ...
        $this->notification = new OfferExpiredNotification($this->offer);
    }
    ...
    // ── Case 6: toDatabase()['type'] equals 'offer_expired' ──────────────────

    public function test_to_database_type_is_offer_expired(): void
    {
        $data = $this->notification->toDatabase(null);

        $this->assertSame('offer_expired', $data['type']);
    }
