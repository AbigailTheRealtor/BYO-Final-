...
/**
 * PublicGeometryProjection — unit contract.
 *
 ...
 *   while retaining the published administrative names that are publishable on
 *   a public surface.
 *
 * Also enforced:
 *   - Output never varies by session, user or login state. There is no parameter
 *     by which it could: project() takes only the array.
 *   - Notes are withheld, never truncated, summarised or inspected.
 *
...
class PublicGeometryProjectionTest extends TestCase
{
    ...
    public function test_absent_keys_are_not_invented(): void
    {
        ...
        // Absence is meaningful in the stored blob. The projection must not
        // manufacture a present-but-empty dimension where the source had none.
    ...
    public function test_projection_is_idempotent_and_preserves_flags(): void
    {
        ...
        // The trap this guards: re-measuring an already-emptied array would flip
        // the withheld flags to false and silently drop the indicator.
    ...
    public function test_strip_from_meta_bag_removes_only_the_blob(): void
    {
        $meta = [
            'listing_title'            => 'A Listing',
            'location_dna_preferences' => $this->sensitivePreferences(),
            'cities'                   => ['Tampa, FL'],
        ];

        $out = $this->projection->stripFromMetaBag($meta);

        $this->assertArrayNotHasKey('location_dna_preferences', $out);
        $this->assertSame('A Listing', $out['listing_title']);
        $this->assertSame(['Tampa, FL'], $out['cities']);
    }

    public function test_strip_from_meta_bag_does_not_mutate_the_caller(): void
    {
        $meta   = ['location_dna_preferences' => ['polygons' => [['path' => []]]]];
        $before = $meta;

        $this->projection->stripFromMetaBag($meta);

        $this->assertSame($before, $meta);
    }

    public function test_strip_from_meta_bag_is_a_no_op_when_the_key_is_absent(): void
    {
        $meta = ['listing_title' => 'A Listing'];

        $this->assertSame($meta, $this->projection->stripFromMetaBag($meta));
    }
}
