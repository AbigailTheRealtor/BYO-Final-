...
| `HIRE_AGENT_DETAIL_REDESIGN_ENABLED` | Master gate for the redesigned Hire Agent listing **detail page** (M5) — section navigation, quick actions, sidebar, cards, photo gallery. Default `false` = inert. Read only via `HireAgentDetailRedesign::enabled()`. **Independent of the hero flag on purpose**: the hero is being visually verified while the detail rebuild is still in progress, so one must not move the other. No role allowlist — the redesign's markup lives in the landlord role view, so role scope is enforced by which files exist; add one here mirroring the hero flag when a second role adopts it. |
