...
**Nothing here activates the corpus.** `OVERTURE_CORPUS_POI_ENABLED` (the adapter gate) and
`OVERTURE_CORPUS_POI_ROUTING_ENABLED` (the registry gate behind
`location_providers.providers.overture_corpus.enabled`) both default `false`, no corpus version is
pinned, and `OvertureActivationReadinessTest` fails if any of that changes. What DID change is the
fallback: with the gates off, `poi.default` now resolves to no provider and a POI run is refused,
rather than resolving to `google_places` by elimination.
